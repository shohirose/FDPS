...
struct TargetBox<SEARCH_MODE_SCATTER> {
  ...
  inline bool isInEpList(const ReallocatableArray<Tep>& ep_first,
    ...
    const F64 r_crit_sq =
        ep_first[adr].getRSearch() * ep_first[adr].getRSearch();
...
struct TargetBox<SEARCH_MODE_SYMMETRY> {
  ...
  inline bool isInEpList(const ReallocatableArray<Tep>& ep_first,
    ...
    const F64 r_crit_sq =
        ep_first[adr].getRSearch() * ep_first[adr].getRSearch();
    return (dis_sq <= r_crit_sq) ||
           (vertex_out_.overlapped(ep_first[adr].getPos()));
  }
};
template <>
...
template <class TSM>
inline void SetTargetBoxExLet(TargetBox<TSM>& box, const F64ort& in,
                              const F64ort& out) {
  assert(0);
}
...
inline void SetTargetBoxExLet<SEARCH_MODE_LONG_SYMMETRY>(
    TargetBox<SEARCH_MODE_LONG_SYMMETRY>& box, const F64ort& in,
    const F64ort& out) {
  box.vertex_in_ = in;
  box.vertex_out_ = out;
}
...
inline bool IsOpen(TagSearchLong, const ReallocatableArray<Ttc>& tc_first,
                   const S32 adr_tc, const TargetBox<TSM>& target_box,
                   const F64 r_crit_sq, const F64vec& len_peri,
                   TagWalkModeNormal) {
...
inline bool IsOpen(TagSearchLongScatter,
                   ...
                   const TargetBox<TSM>& target_box, const F64 r_crit_sq,
                   const F64vec& len_peri, TagWalkModeNormal) {
  return (tc_first[adr_tc].n_ptcl_ > 0);
}
...
template <class TSM, class Ttc>
inline bool IsOpen(TagSearchLongCutoff, const ReallocatableArray<Ttc>& tc_first,
                   const S32 adr_tc, const TargetBox<TSM>& target_box,
                   const F64 r_crit_sq, const F64vec& len_peri,
                   TagWalkModeNormal) {
  return (
      (target_box.vertex_.overlapped(tc_first[adr_tc].mom_.getVertexOut())) &&
...
inline bool IsOpen(TagSearchShortScatter,
                   ...
                   const TargetBox<TSM>& target_box, const F64 r_crit_sq,
                   const F64vec& len_peri, TagWalkModeNormal) {
  return ((target_box.vertex_in_.overlapped(
              tc_first[adr_tc].mom_.getVertexOut())) &&
          (tc_first[adr_tc].n_ptcl_ > 0));
}
...
inline bool IsOpen(TagSearchShortSymmetry,
  ...
  return (((target_box.vertex_in_.overlapped(
               tc_first[adr_tc].mom_.getVertexOut())) ||
           (target_box.vertex_out_.overlapped(
               tc_first[adr_tc].mom_.getVertexIn()))) &&
          (tc_first[adr_tc].n_ptcl_ > 0));
}
...
inline void CopyInfoDistant(TagForceLong, const S32 adr_sp,
                            ReallocatableArray<S32>& adr_sp_list) {
...
inline void CopyInfoClose(TagForceLong, TagChopLeafTrue, TagCopyInfoCloseNoSp,
                          const ReallocatableArray<Ttp>& tp_first,
                          const S32 adr_ptcl, const S32 n_ptcl,
...
template <class TSM, class Ttp, class Tep, class Tsp>
inline void CopyInfoClose(TagForceLong, TagChopLeafFalse, TagCopyInfoCloseNoSp,
                          const ReallocatableArray<Ttp>& tp_first,
                          const S32 adr_ptcl, const S32 n_ptcl,
                          const ReallocatableArray<Tep>& ep_first,
                          ReallocatableArray<S32>& adr_ep_list,
...
template <class TSM, class Ttp, class Tep, class Tsp>
inline void CopyInfoClose(TagForceLong, TagChopLeafTrue,
...
template <class TSM, class Ttp, class Tep, class Tsp>
inline void CopyInfoClose(TagForceLong, TagChopLeafFalse,
                          TagCopyInfoCloseWithTpAdrptcl,
                          const ReallocatableArray<Ttp>& tp_first,
                          const S32 adr_ptcl, const S32 n_ptcl,
                          const ReallocatableArray<Tep>& ep_first,
...
template <class TSM, class Ttp, class Tep, class Tsp>
inline void CopyInfoClose(TagForceShort, TagChopLeafTrue, TagCopyInfoCloseNoSp,
                          const ReallocatableArray<Ttp>& tp_first,
                          const S32 adr_ptcl, const S32 n_ptcl,
                          const ReallocatableArray<Tep>& ep_first,
                          ReallocatableArray<S32>& adr_ep_list,
...
template <class TSM, class Ttp, class Tep, class Tsp>
inline void CopyInfoClose(TagForceShort, TagChopLeafFalse, TagCopyInfoCloseNoSp,
                          const ReallocatableArray<Ttp>& tp_first,
                          const S32 adr_ptcl, const S32 n_ptcl,
                          const ReallocatableArray<Tep>& ep_first,
                          ReallocatableArray<S32>& adr_ep_list,
...
// GET OPEN BITS
template <class TSM, class Ttc>
inline U32 GetOpenBit(TagSearchLong, const ReallocatableArray<Ttc>& tc_first,
                      const S32 adr_tc, const TargetBox<TSM>& target_box,
                      const F64 r_crit_sq, const F64vec& len_peri,
                      TagWalkModeNormal) {
  U32 open_bit = 0;
...
inline U32 GetOpenBit(TagSearchLongScatter,
  ...
  for (S32 i = 0; i < N_CHILDREN; i++) {
    // const F64 dis0 =
    // target_box.vertex_.getDistanceMinSq(tc_first[adr_tc+i].mom_.getVertexOut());
    const F64vec pos = tc_first[adr_tc + i].mom_.getPos();
    const F64 dis1 = target_box.vertex_.getDistanceMinSq(pos);
    open_bit |= (target_box.vertex_.overlapped(
...
inline U32 GetOpenBit(TagSearchLongSymmetry,
  ...
  for (S32 i = 0; i < N_CHILDREN; i++) {
    ...
    open_bit |= ((target_box.vertex_in_.overlapped(
                      tc_first[adr_tc + i].mom_.getVertexOut()) ||
                  target_box.vertex_out_.overlapped(
...
inline U32 GetOpenBit(TagSearchShortGather,
  ...
  U32 open_bit = 0;
  for (S32 i = 0; i < N_CHILDREN; i++) {
    open_bit |= (target_box.vertex_out_.overlapped(
                    tc_first[adr_tc + i].mom_.getVertexIn()))
                << i;
    open_bit |= ((tc_first[adr_tc + i].n_ptcl_ > 0)
                 << (i + N_CHILDREN));  // if true, it should be checked
...
template <class TSM, class Ttc, class Ttp, class Tep, class Tsp,
          class Twalkmode, class Tchopleaf, class Tcopyinfoclose>
inline void MakeListUsingTreeRecursive(
    ...
    const S32
        adr_tree_sp_first,  // adress of first sp coming from the (global) tree.
  ...
  if (!(tc_cur->isLeaf(n_leaf_limit))) {  // not leaf
    U32 open_bit = GetOpenBit(typename TSM::search_type(), tc_first,
                              adr_tc_child, target_box, r_crit_sq * 0.25,
                              len_peri, typename Twalkmode::walk_type());
    for (S32 i = 0; i < N_CHILDREN; i++) {
      if (!((open_bit >> (i + N_CHILDREN)) & 0x1))
        ...
        MakeListUsingTreeRecursive<TSM, Ttc, Ttp, Tep, Tsp, Twalkmode,
            ...
            tc_first, adr_tc_child + i, tp_first, ep_first, adr_ep_list,
            sp_first, adr_sp_list, target_box, r_crit_sq * 0.25, n_leaf_limit,
            adr_tree_sp_first, len_peri);
      } else {  // far
        CopyInfoDistant(typename TSM::force_type(),
                        adr_tc_child + adr_tree_sp_first + i, adr_sp_list);
      }
    }
  } else {  // leaf
    CopyInfoClose(typename TSM::force_type(), Tchopleaf(), Tcopyinfoclose(),
                  tp_first, adr_ptcl_child, n_ptcl, ep_first, adr_ep_list,
                  sp_first, adr_sp_list, target_box);
  }
}
...
inline void MakeListUsingTreeRecursiveTop(
    ...
    ReallocatableArray<S32>& adr_ep_list,
    const ReallocatableArray<Tsp>& sp_first,
    ReallocatableArray<S32>& adr_sp_list, const TargetBox<TSM>& target_box,
  ...
  if ((r_crit_sq >= 0.0) || (typeid(TSM) == typeid(SEARCH_MODE_LONG_CUTOFF))) {
    ...
    if (IsOpen(typename TSM::search_type(), tc_first, adr_tc, target_box,
      ...
      MakeListUsingTreeRecursive<TSM, Ttc, Ttp, Tep, Tsp, Twalkmode, Tchopleaf,
          ...
          adr_sp_list, target_box, r_crit_sq, n_leaf_limit, adr_tree_sp_first,
          len_peri);
    }
  } else {
    ...
    CopyInfoClose(typename TSM::force_type(), Tchopleaf(), Tcopyinfoclose(),
                  tp_first, adr_ptcl, n_ptcl, ep_first, adr_ep_list, sp_first,
                  adr_sp_list, target_box);
  }
}
...
inline void MakeListUsingTreeRecursiveTop(
    ...
    const ReallocatableArray<Ttp>& tp_first,
    const ReallocatableArray<Tep>& ep_first,
    ReallocatableArray<S32>& adr_ep_list, const TargetBox<TSM>& target_box,
    const F64 r_crit_sq, const S32 n_leaf_limit, const F64vec& len_peri) {
  ...
  MakeListUsingTreeRecursive<TSM, Ttc, Ttp, Tep, SuperParticleBase, Twalkmode,
                             Tchopleaf, TagCopyInfoCloseNoSp>(
      tc_first, adr_tc, tp_first, ep_first, adr_ep_list, sp_first, adr_sp_list,
      target_box, r_crit_sq, n_leaf_limit, adr_tree_sp_first, len_peri);
  // adr_sp_list.freeMem();
  // sp_first.freeMem();
}
...
template <class Ttc>
inline void IsOverlapped(const Ttc* tc_first, const S32 adr_tc,
                         const F64ort& pos_box, const S32 n_leaf_limit,
                         U32& is_overlapped) {
  if (is_overlapped) return;
  U32 open_bit = 0;
  for (S32 i = 0; i < N_CHILDREN; i++) {
    open_bit |=
        (tc_first[adr_tc + i].mom_.getVertexOut().overlapped(pos_box) << i);
  }
  for (S32 i = 0; i < N_CHILDREN; i++) {
    if (is_overlapped) return;
    ...
    if ((open_bit >> i) & 0x1) {
      if (!(tc_child->isLeaf(n_leaf_limit))) {
        // not leaf
        IsOverlapped(tc_first, tc_first[adr_tc_child].adr_tc_, pos_box,
...
inline U32 IsOverlappedTop(const ReallocatableArray<Ttc>& tc_first,
  ...
  if (!tc_first[0].isLeaf(n_leaf_limit)) {
    IsOverlapped(tc_first.getPointer(), adr_tc, pos_box, n_leaf_limit,
                 is_overlapped);
  } else {
...
inline U32 GetOpenBitDoubleWalk(const ReallocatableArray<Ttca>& tc_first_A,
                                ...
                                const S32 adr_tc_B, const F64ort& pos_domain,
                                const S32 n_leaf_limit_A) {
  U32 open_bit = 0;
  for (S32 i = 0; i < N_CHILDREN; i++) {
    open_bit |=
        (tc_first_B[adr_tc_B + i].mom_.getVertexOut().overlapped(pos_domain) ||
         IsOverlappedTop(tc_first_A,
                         tc_first_B[adr_tc_B + i].mom_.getVertexIn(),
                         n_leaf_limit_A))
        << i;
    open_bit |= (tc_first_B[adr_tc_B + i].n_ptcl_ > 0)
                << (i + N_CHILDREN);  // if false, just skip
  }
  return open_bit;
}
...
inline void MakeListDoubleWalk(const ReallocatableArray<Ttca>& tc_first_A,
  ...
  U32 open_bit = GetOpenBitDoubleWalk(tc_first_A, tc_first_B, adr_tc_B,
                                      pos_domain, n_leaf_limit_A);
  for (S32 i = 0; i < N_CHILDREN; i++) {
    if (!((open_bit >> (i + N_CHILDREN)) & 0x1))
      continue;
      ...
      if (!(tc_child->isLeaf(n_leaf_limit_B))) {
        MakeListDoubleWalk(tc_first_A, tc_first_B,
                           tc_first_B[adr_tc_child].adr_tc_, ep_first_B,
                           pos_domain, n_leaf_limit_A, n_leaf_limit_B,
                           adr_ptcl_send);
      } else {
        if (pos_domain.overlapped(
...
inline void MakeListDoubleWalkTop(
    ...
    const ReallocatableArray<Tepb>& ep_first_B, const F64ort& pos_domain,
    const S32 n_leaf_limit_A, const S32 n_leaf_limit_B,
    ReallocatableArray<S32>& adr_ptcl_send) {
  const S32 adr_tc_B = N_CHILDREN;
  if (!tc_first_B[0].isLeaf(n_leaf_limit_B)) {
    MakeListDoubleWalk(tc_first_A, tc_first_B, adr_tc_B, ep_first_B, pos_domain,
                       n_leaf_limit_A, n_leaf_limit_B, adr_ptcl_send);
  } else {
    // original
