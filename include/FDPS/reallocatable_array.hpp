...
class ReallocatableArray {
 private:
  ...
  int roundUp(const int cap_prev, const int factor = 8) {
  ...
  void checkCapacityOverInConstructor() {
    if (capacity_ >= LIMIT_NUMBER_OF_TREE_PARTICLE_PER_NODE) {
      PARTICLE_SIMULATOR_PRINT_ERROR(
          "The number of particles of this process is beyound the FDPS limit "
          "number");
      ...
      std::cerr << "LIMIT_NUMBER_OF_TREE_PARTICLE_PER_NODE="
                << LIMIT_NUMBER_OF_TREE_PARTICLE_PER_NODE << std::endl;
      Abort(-1);
    }
  }
...
#pragma omp critical
 ...
 public:
  ...
  /*
  ReallocatableArray(int cap) : data_(NULL), size_(0), capacity_org_(0),
  id_mpool_(-1), alloc_mode_(0), n_expand_(0) { constructorFunction(cap);
  }

  ReallocatableArray(int cap, int size) : data_(NULL), size_(size),
  capacity_org_(0), id_mpool_(-1), alloc_mode_(0), n_expand_(0) {
      constructorFunction(cap);
  }
  */

  ReallocatableArray(int cap, int size, int alloc_mode)
      : data_(NULL),
        size_(size),
        capacity_org_(0),
        id_mpool_(-1),
  ...
  /*
  ReallocatableArray(int cap) : data_(NULL), size_(0), capacity_org_(0),
  id_mpool_(-1), alloc_mode_(0) { constructorFunction(cap);
  }

  ReallocatableArray(int cap, int size) : data_(NULL), size_(size),
  capacity_org_(0), id_mpool_(-1), alloc_mode_(0) { constructorFunction(cap);
  }
  */
...
#pragma omp critical
  ...
  void setAllocMode(const int alloc_mode) { alloc_mode_ = alloc_mode; }
...
#endif
    const int new_cap =
        std::max(getNewCapacity((n + (n + 10) / 10) + 100), capacity_org_);
    if (data_ == NULL) {
      if (alloc_mode_ == 0) {
        data_ = new T[new_cap];
      } else if (alloc_mode_ == 1) {
...
//#ifdef SANITY_CHECK_REALLOCATABLE_ARRAY
  ...
  void decreaseSize(const int n = 1) {
    resizeNoInitialize(size_ - n);
  }  // no check
    ...
    if (alloc_mode_ == 0 && (free_alloc_mode == -1 || free_alloc_mode == 0)) {
      if (capacity_ > 0) {
        ...
        capacity_org_ = 0;
               ...
               (free_alloc_mode == -1 || free_alloc_mode == 1)) {
