...
__artificial__)) _mm256_undefined_ps (void)
{
  __m256 __Y = __Y;
  return __Y;
}
...
static inline void transpose_4ymm(__m256 &v0, __m256 &v1, __m256 &v2,
                                  __m256 &v3) {
  // transpose from
  // [ |w4|z4|y4|x4||w0|z0|y0|x0|, |w5|z5|y5|x5||w1|z1|y1|x1|,
  // |w6|z6|y6|x6||w2|z2|y2|x2|, |w7|z7|y7|x7||w3|z3|y3|x3| ] to [
  // |x7|x6|x5|x4||x3|x2|x1|x0|, |y7|y6|y5|y4||y3|y2|y1|y0|,
  // |z7|z6|z5|z4||z3|z2|z1|z0|, |w7|w6|w5|w4||w3|w2|w1|w0| ]
...
inline void CalcNumberAndShiftOfImageDomain(
    ...
    const F64vec &size_root_domain, const F64ort &pos_my_domain,
    const F64ort &pos_target_domain, const bool periodic_axis[],
...
inline void AllGatherParticle(Tptcl *&ptcl, S32 *&n_ptcl, S32 *&n_ptcl_disp,
                              const Tptcl ptcl_org[], const S32 n_loc) {
...
template <class Tptcl>
inline void AllGatherParticle(Tptcl *&ptcl, S32 &n_tot, const Tptcl ptcl_org[],
                              const S32 n_loc) {
...
inline void AllGatherParticle(Tptcl *&ptcl, S32 *&n_ptcl, S32 *&n_ptcl_disp,
  ...
  CalcNumberAndShiftOfImageDomain(shift, domain_size, pos_root_tree,
                                  pos_root_domain, periodic_axis);
  ...
  std::cerr<<"n_image= "<<n_image
           ...
           <<" periodic_axis= "<<periodic_axis[0]<<" "<<periodic_axis[1]<<"
  "<<periodic_axis[2]
           <<std::endl;
  */
...
template <class Tptcl>
inline void AllGatherParticle(Tptcl *&ptcl, S32 &n_tot, const Tptcl ptcl_org[],
                              const S32 n_loc, const F64ort &pos_root_domain,
                              const F64ort &pos_root_cell,
                              const bool periodic_axis[]) {
  S32 *n_tmp;
  S32 *n_disp_tmp;
  AllGatherParticle(ptcl, n_tmp, n_disp_tmp, ptcl_org, n_loc, pos_root_domain,
                    pos_root_cell, periodic_axis);
  n_tot = n_disp_tmp[Comm::getNumberOfProc()];
  delete[] n_tmp;
  delete[] n_disp_tmp;
}
...
// binary search
template <class Ttp>
inline S32 GetPartitionID(const Ttp tp[], const S32 left, const S32 right,
                          const S32 ref, const S32 lev) {
  // if( MortonKey<DIMENSION>::getCellID(lev, tp[right].getKey()) < ref) return
  // -1; if( MortonKey<DIMENSION>::getCellID(lev, tp[left].getKey()) > ref)
  // return -1;
  ...
  while (l < r) {
    ...
    // if( MortonKey<DIMENSION>::getCellID(lev, tp[l].getKey()) == ref ){ return
    // l;
...
inline void LinkCell(ReallocatableArray<Ttc> &tc_array,
                     S32 adr_tc_level_partition[], const TreeParticle tp[],
                     S32 &lev_max, const S32 n_tot, const S32 n_leaf_limit) {
...
#pragma omp parallel for reduction(+ : n_cell_new)
    ...
    for (S32 i = id_cell_left; i < id_cell_right + 1; i++) {
      ...
      for (S32 j = 0; j < N_CHILDREN; j++) {
        ...
        tc_array[adr_tc_tmp].adr_ptcl_ = GetPartitionID(
            tp, adr_ptcl_tmp, adr_ptcl_tmp + n_ptcl_tmp - 1, j, lev_max + 1);
      ...
      //    //    std::cout << "j = " << j << " "
      //    //              << "tc_array[].adr_ptcl_ = " <<
      //    tc_array[adr_tc_tmp].adr_ptcl_ << " "
      //    //              << "tc_array[].level_ = " <<
      //    tc_array[adr_tc_tmp].level_ << " "
      //    //              << std::endl;
      //    //}
      ...
      //                  << " "
      //                  << "val    = " <<
      //                  MortonKey::getCellID(lev_max+1,tp[ip].key_) << " "
      ...
      for (S32 j = 0; j < n_cnt - 1; j++) {
        tc_array[adr[j]].n_ptcl_ =
            tc_array[adr[j + 1]].adr_ptcl_ - tc_array[adr[j]].adr_ptcl_;
...
inline void CalcMoment(S32 adr_tc_level_partition[], Ttc tc[], Tepj epj[],
                       const S32 lev_max, const S32 n_leaf_limit) {
...
template <class Ttc, class Tepj>
inline void CalcMomentST(S32 adr_tc_level_partition[], Ttc tc[], Tepj epj[],
                         const S32 lev_max, const S32 n_leaf_limit) {
  for (S32 i = lev_max; i >= 0; --i) {
    const S32 head = adr_tc_level_partition[i];
    const S32 next = adr_tc_level_partition[i + 1];
...
inline void CalcMomentLongGlobalTree(S32 adr_tc_level_partition[], Ttc tc[],
                                     TreeParticle tp[], Tepj epj[], Tspj spj[],
                                     const S32 lev_max,
...
inline void MakeIPGroupLong(ReallocatableArray<Tipg> &ipg_first,
  ...
  if (n_tmp == 0)
    ...
    // ipg_first.back().vertex_ =
    // GetMinBoxSingleThread(epi_first.data()+(tc_tmp->adr_ptcl_), n_tmp);
    return;
  } else {
    S32 adr_tc_tmp = tc_tmp->adr_tc_;
    for (S32 i = 0; i < N_CHILDREN; i++) {
      MakeIPGroupLong(ipg_first, tc_first, epi_first, adr_tc_tmp + i,
                      n_grp_limit);
...
inline void MakeIPGroupLongGLBTreeCellAsIPGBox(
    ...
    const ReallocatableArray<Tepj> &epj_first, const S32 adr_tc_loc,
  ...
  if (n_loc_tmp == 0 || n_glb_tmp == 0)
    return;
  else if (tc_glb_tmp->isLeaf(n_grp_limit) ||
           tc_loc_tmp->isLeaf(n_leaf_limit)) {
    ipg_first.increaseSize();
    ipg_first.back().copyFromTC(*tc_loc_tmp);
    const S32 adr_tmp = tc_glb_tmp->adr_ptcl_;
    ipg_first.back().vertex_ =
        GetMinBoxSingleThread(epj_first.getPointer(adr_tmp), n_glb_tmp);
    return;
  } else {
    ...
    for (S32 i = 0; i < N_CHILDREN; i++) {
      MakeIPGroupLongGLBTreeCellAsIPGBox(
          ipg_first, tc_loc_first, tc_glb_first, epj_first, adr_tc_loc_tmp + i,
          adr_tc_glb_tmp + i, n_grp_limit, n_leaf_limit);
    }
  }
}
//#endif
...
inline void MakeIPGroupUseGLBTreeLong(
    ...
    const ReallocatableArray<Ttcloc> &tc_loc_first,
    const ReallocatableArray<Ttcglb> &tc_glb_first,
    const ReallocatableArray<Tepi> &epi_first, const S32 adr_tc_loc,
    const S32 adr_tc_glb, const S32 n_grp_limit, const S32 n_leaf_limit) {
  const Ttcloc *tc_loc_tmp = tc_loc_first.getPointer(adr_tc_loc);
  const Ttcglb *tc_glb_tmp = tc_glb_first.getPointer(adr_tc_glb);
  const S32 n_loc_tmp = tc_loc_tmp->n_ptcl_;
  ...
  if (n_loc_tmp == 0 || n_glb_tmp == 0)
    ...
    // ipg_first.back().vertex_ =
    // GetMinBoxSingleThread(epi_first.getPointer(adr_tmp), n_loc_tmp);
    return;
  } else {
    S32 adr_tc_loc_tmp = tc_loc_tmp->adr_tc_;
    S32 adr_tc_glb_tmp = tc_glb_tmp->adr_tc_;
    for (S32 i = 0; i < N_CHILDREN; i++) {
      MakeIPGroupUseGLBTreeLong(ipg_first, tc_loc_first, tc_glb_first,
                                epi_first, adr_tc_loc_tmp + i,
                                adr_tc_glb_tmp + i, n_grp_limit, n_leaf_limit);
    }
  }
}
...
inline void MakeIPGroupShort(ReallocatableArray<Tipg> &ipg_first,
  ...
  if (n_tmp == 0)
    ...
    S32 adr_tc_tmp = tc_tmp->adr_tc_;
    for (S32 i = 0; i < N_CHILDREN; i++) {
      MakeIPGroupShort<Tipg, Ttc, Tepi>(ipg_first, tc_first, epi_first,
                                        adr_tc_tmp + i, n_grp_limit);
    }
  }
}

template <class Tep>
void CheckMortonSort(const S32 n, TreeParticle tp[], Tep ep[],
  ...
  if (err_comp || err_order) {
    fout << "checkMortonSort: FAIL err_comp=" << err_comp
         << " err_order=" << err_order << std::endl;
  } else {
...
void CheckMortonSort(const S32 n, TreeParticle tp[], Tep ep[], Tsp sp[],
                     std::ostream &fout = std::cout) {
  S32 err_comp = 0;
  S32 err_order = 0;
  F64 mass_cm_tmp = 0.0;
...
void CheckCalcMomentLongLocalTree(Tep ep[], const S32 n_ptcl, Ttc tc[],
  ...
  if (Comm::getRank() == 0) {
    fout << "mass_cm=" << mass_cm << ", tc[0].mom_.mass=" << tc[0].mom_.mass
         << std::endl;
    fout << "pos_cm=" << pos_cm << ", tc[0].mom_.pos=" << tc[0].mom_.pos
         << std::endl;
  }
}

template <class Tep, class Tsp, class Ttc>
void CheckCalcMomentLongGlobalTree(const Tep ep[], const Tsp sp[],
                                   const S32 n_ptcl, const Ttc tc[],
  ...
  if (Comm::getRank() == 0) {
    fout << "mass_cm_glb=" << mass_cm_glb
         << ", tc[0].mom_.mass=" << tc[0].mom_.mass << std::endl;
    fout << "pos_cm_glb=" << pos_cm_glb << ", tc[0].mom_.pos=" << tc[0].mom_.pos
         << std::endl;
  }
}

template <class Tep, class Ttc>
void CheckCalcMomentShortInAndOut(Tep ep[], const S32 n_ptcl, Ttc tc[],
                                  const F64 tolerance = 1e-5,
                                  std::ostream &fout = std::cout) {
  ...
  F64 d_out_high = (tc[0].mom_.vertex_out_.high_ - vertex_out_tmp.high_)
                       .applyEach(Abs<F64>())
                       .getMax();
  F64 d_out_low = (tc[0].mom_.vertex_out_.low_ - vertex_out_tmp.low_)
                      .applyEach(Abs<F64>())
                      .getMax();
  F64 d_in_high = (tc[0].mom_.vertex_in_.high_ - vertex_in_tmp.high_)
                      .applyEach(Abs<F64>())
                      .getMax();
  F64 d_in_low = (tc[0].mom_.vertex_in_.low_ - vertex_in_tmp.low_)
                     .applyEach(Abs<F64>())
                     .getMax();
  if (d_out_high >= tolerance || d_out_low >= tolerance ||
      d_in_high >= tolerance || d_in_low >= tolerance) {
    fout << "CalcMomentShort test: FAIL" << std::endl;
    fout << "vertex_out_tmp.high_=" << vertex_out_tmp.high_
         << " tc[0].mom_.vertex_out_.high_=" << tc[0].mom_.vertex_out_.high_
         << std::endl;
    fout << "vertex_out_tmp.low_=" << vertex_out_tmp.low_
         << " tc[0].mom_.vertex_out_.low_=" << tc[0].mom_.vertex_out_.low_
         << std::endl;
    fout << "vertex_in_tmp.high_=" << vertex_in_tmp.high_
         << " tc[0].mom_.vertex_in_.high_=" << tc[0].mom_.vertex_in_.high_
         << std::endl;
    fout << "vertex_in_tmp.low_=" << vertex_in_tmp.low_
         << " tc[0].mom_.vertex_in_.low_=" << tc[0].mom_.vertex_in_.low_
         << std::endl;
  } else {
...
template <class Tep, class Ttc>
void CheckCalcMomentShortInOnly(Tep ep[], const S32 n_ptcl, Ttc tc[],
                                const F64 tolerance = 1e-5,
                                std::ostream &fout = std::cout) {
  F64ort vertex_in_tmp;
  vertex_in_tmp.init();
...
template <class Tep, class Ttc>
void CheckCalcMomentOutOnly(Tep ep[], const S32 n_ptcl, Ttc tc[],
                            const F64 tolerance = 1e-5,
                            std::ostream &fout = std::cout) {
  F64ort vertex_out_tmp;
  vertex_out_tmp.init();
...
inline void SearchSendParticleLong(
    const ReallocatableArray<Ttc> &tc_first, const S32 adr_tc,
    const ReallocatableArray<Tep> &ep_first,
    ReallocatableArray<S32> &id_ep_send, ReallocatableArray<S32> &id_sp_send,
    const F64ort &pos_target_box,  // position of domain
    const F64 r_crit_sq, const S32 n_leaf_limit) {
  U32 open_bits = 0;
  for (S32 i = 0; i < N_CHILDREN; i++) {
    ...
    if (n_child == 0)
      ...
      if (!(tc_child->isLeaf(n_leaf_limit))) {
        ...
        SearchSendParticleLong<Ttc, Tep>(
            tc_first, tc_first[adr_tc_child].adr_tc_, ep_first, id_ep_send,
            id_sp_send, pos_target_box, r_crit_sq, n_leaf_limit);
#else
        SearchSendParticleLong<Ttc, Tep>(
            tc_first, tc_first[adr_tc_child].adr_tc_, ep_first, id_ep_send,
            id_sp_send, pos_target_box, r_crit_sq * 0.25, n_leaf_limit);
#endif
      } else {
...
// FOR P^3T
template <class Ttc, class Tep>
inline void SearchSendParticleLongScatter(
    const ReallocatableArray<Ttc> &tc_first, const S32 adr_tc,
    const ReallocatableArray<Tep> &ep_first,
    ReallocatableArray<S32> &id_ep_send, ReallocatableArray<S32> &id_sp_send,
    const F64ort &pos_target_box,  // position of domain
...
template <class Ttc, class Tep>
inline void SearchSendParticleLongSymmetry(
    const ReallocatableArray<Ttc> &tc_first, const S32 adr_tc,
    const ReallocatableArray<Tep> &ep_first,
    ReallocatableArray<S32> &id_ep_send, ReallocatableArray<S32> &id_sp_send,
    const F64ort &pos_target_box_in, const F64ort &pos_target_box_out,
    const F64 r_crit_sq, const S32 n_leaf_limit) {
  U32 open_bits = 0;
  for (S32 i = 0; i < N_CHILDREN; i++) {
    const F64vec pos_tmp = tc_first[adr_tc + i].mom_.getPos();
    ...
    open_bits |= ( ( (pos_target_box_in.getDistanceMinSQ(pos_tmp) <= r_crit_sq)
                     || (pos_target_box_in.contained(
    tc_first[adr_tc+i].mom_.getVertexOut()))
                     || (pos_target_box_out.contained(
    tc_first[adr_tc+i].mom_.getVertexIn())) )
                   << i);
    */
    open_bits |= (((pos_target_box_in.getDistanceMinSQ(pos_tmp) <= r_crit_sq) ||
                   (pos_target_box_in.overlapped(
                       tc_first[adr_tc + i].mom_.getVertexOut())) ||
                   (pos_target_box_out.overlapped(
                       tc_first[adr_tc + i].mom_.getVertexIn())))
                  << i);
  }
  for (S32 i = 0; i < N_CHILDREN; i++) {
    ...
    if (n_child == 0)
      ...
      if (!(tc_child->isLeaf(n_leaf_limit))) {
        SearchSendParticleLongSymmetry<Ttc, Tep>(
            tc_first, tc_first[adr_tc_child].adr_tc_, ep_first, id_ep_send,
            id_sp_send, pos_target_box_in, pos_target_box_out, r_crit_sq * 0.25,
            n_leaf_limit);
      } else {
        S32 adr_ptcl_tmp = tc_child->adr_ptcl_;
        id_ep_send.reserveEmptyAreaAtLeast(n_child);
...
inline F64ort makeChildCellBox(const S32 i, const F64ort &cell_box) {
  ...
  for (S32 k = 0; k < DIMENSION; k++) {
    ...
    child_cell_low[k] =
        cell_box.low_[k] + ((i >> (DIMENSION - 1 - k)) & 0x1) * half_length;
  }
...
inline void SearchSendParticleLongCutoff(
    const ReallocatableArray<Ttc> &tc_first, const S32 adr_tc,
    const ReallocatableArray<Tep> &ep_first, ReallocatableArray<Tep> &ep_send,
    ReallocatableArray<Tsp> &sp_send, const F64ort &cell_box,
    const F64ort &pos_target_domain, const F64 r_crit_sq, const F64 r_cut_sq,
    const S32 n_leaf_limit, const F64vec &shift = 0.) {
  U32 open_bits = 0;
  for (S32 i = 0; i < N_CHILDREN; i++) {
    F64vec pos_tmp = tc_first[adr_tc + i].mom_.getPos();
    open_bits |= ((pos_target_domain.getDistanceMinSQ(pos_tmp) <= r_crit_sq)
                  << i);  // using opening criterion
#if 0
    ...
    open_bits |=
        (pos_target_domain.contained(tc_first[adr_tc + i].mom_.getVertexOut())
         << (i + N_CHILDREN));
#endif
  }
  for (S32 i = 0; i < N_CHILDREN; i++) {
    ...
    if (n_child == 0)
      ...
      if (!(tc_child->isLeaf(n_leaf_limit))) {
        ...
        SearchSendParticleLongCutoff<Ttc, Tep, Tsp>(
            tc_first, tc_first[adr_tc_child].adr_tc_, ep_first, ep_send,
            sp_send, child_cell_box, pos_target_domain, r_crit_sq * 0.25,
            r_cut_sq, n_leaf_limit, shift);
      } else {
        S32 adr_ptcl_tmp = tc_child->adr_ptcl_;
...
inline void SearchSendParticleLongCutoffWithRootCellCheck(
    ...
    ReallocatableArray<Tsp> &sp_send, const F64ort &cell_box,
    const F64ort &pos_target_domain, const F64 r_crit_sq, const F64 r_cut_sq,
    const S32 n_leaf_limit, const F64ort pos_root_cell,
  ...
  for (S32 i = 0; i < N_CHILDREN; i++) {
    ...
    const F64ort child_cell_box = makeChildCellBox(i, cell_box);
    open_bits |=
        ((pos_target_domain.getDistanceMinSQ(child_cell_box) <= r_cut_sq)
         << (i + N_CHILDREN));  // using cutoff criterion
#else
    // cell_box is not neede
    open_bits |=
        (pos_target_domain.contained(tc_first[adr_tc + i].mom_.getVertexOut())
    ...
    if (n_child == 0)
      ...
      if (!(tc_child->isLeaf(n_leaf_limit))) {
        ...
        SearchSendParticleLongCutoffWithRootCellCheck<Ttc, Tep, Tsp>(
            tc_first, tc_first[adr_tc_child].adr_tc_, ep_first, ep_send,
            sp_send, child_cell_box, pos_target_domain, r_crit_sq * 0.25,
            r_cut_sq, n_leaf_limit, pos_root_cell, shift);
      } else {
        S32 adr_ptcl_tmp = tc_child->adr_ptcl_;
        // ep_send.reserve( ep_send.size()+n_child );
        ep_send.reserveEmptyAreaAtLeast(n_child);
...
inline void SearchSendParticleLongCutoffForZeroTheta(
    const ReallocatableArray<Ttc> &tc_first, const S32 adr_tc,
    const ReallocatableArray<Tep> &ep_first, ReallocatableArray<Tep> &ep_send,
    const F64ort &cell_box, const F64ort &pos_target_domain, const F64 r_cut_sq,
    const S32 n_leaf_limit, const F64ort pos_root_cell,
    const F64vec &shift = 0.) {
  U32 open_bits = 0;
  for (S32 i = 0; i < N_CHILDREN; i++) {
    const F64ort child_cell_box = makeChildCellBox(i, cell_box);
    open_bits |=
        ((pos_target_domain.getDistanceMinSQ(child_cell_box) <= 1.01 * r_cut_sq)
         << i);
    ...
    if (n_child == 0)
      ...
      if (!(tc_child->isLeaf(n_leaf_limit))) {
        ...
        SearchSendParticleLongCutoffForZeroTheta<Ttc, Tep>(
            tc_first, tc_first[adr_tc_child].adr_tc_, ep_first, ep_send,
            child_cell_box, pos_target_domain, r_cut_sq, n_leaf_limit,
            pos_root_cell, shift);
      } else {
        S32 adr_ptcl_tmp = tc_child->adr_ptcl_;
        ep_send.reserveEmptyAreaAtLeast(n_child);
...
void MakeInteractionListLongEPSP(
    const ReallocatableArray<Ttc> &tc_first, const S32 adr_tc,
    const ReallocatableArray<Ttp> &tp_first,
    const ReallocatableArray<Tep> &ep_first, ReallocatableArray<Tep> &ep_list,
    const ReallocatableArray<Tsp> &sp_first, ReallocatableArray<Tsp> &sp_list,
    const F64ort &pos_target_box, const F64 r_crit_sq, const S32 n_leaf_limit) {
  U32 open_bits = 0;
//#ifdef __HPC_ACE__
#ifdef FAST_WALK_K
  ...
  for (S32 i = 0; i < N_CHILDREN / 2; i++) {
    ...
    _fjsp_v2r8 dr_sq_tmp = _fjsp_madd_v2r8(
        dz, dz, _fjsp_madd_v2r8(dy, dy, _fjsp_mul_v2r8(dx, dx)));
  ...
  __m256 jx =
      _mm256_set_m128_forgcc(_mm_loadu_ps(&mom4.x), _mm_loadu_ps(&mom0.x));
  __m256 jy =
      _mm256_set_m128_forgcc(_mm_loadu_ps(&mom5.x), _mm_loadu_ps(&mom1.x));
  __m256 jz =
      _mm256_set_m128_forgcc(_mm_loadu_ps(&mom6.x), _mm_loadu_ps(&mom2.x));
  __m256 jw =
      _mm256_set_m128_forgcc(_mm_loadu_ps(&mom7.x), _mm_loadu_ps(&mom3.x));
    ...
    if (n_child == 0) {
      ...
      if (!(tc_child->isLeaf(n_leaf_limit))) {
        ...
        MakeInteractionListLongEPSP<Ttc, Ttp, Tep, Tsp>(
            ...
            ep_list, sp_first, sp_list, pos_target_box, r_crit_sq,
            n_leaf_limit);
#else
        MakeInteractionListLongEPSP<Ttc, Ttp, Tep, Tsp>(
            tc_first, tc_first[adr_tc_child].adr_tc_, tp_first, ep_first,
            ep_list, sp_first, sp_list, pos_target_box, r_crit_sq * 0.25,
            n_leaf_limit);
#endif
      } else {
...
template <class Ttc, class Ttp, class Tep, class Tsp>
void MakeInteractionListLongSymmetryEPSP(
    const ReallocatableArray<Ttc> &tc_first, const S32 adr_tc,
    const ReallocatableArray<Ttp> &tp_first,
    const ReallocatableArray<Tep> &ep_first, ReallocatableArray<Tep> &ep_list,
    const ReallocatableArray<Tsp> &sp_first, ReallocatableArray<Tsp> &sp_list,
    ...
    if (n_child == 0)
      ...
      if (!(tc_child->isLeaf(n_leaf_limit))) {
        MakeInteractionListLongSymmetryEPSP<Ttc, Ttp, Tep, Tsp>(
            tc_first, tc_first[adr_tc_child].adr_tc_, tp_first, ep_first,
            ep_list, sp_first, sp_list, pos_target_box_in, pos_target_box_out,
        ...
        for (S32 ip = 0; ip < n_child; ip++) {
          if (GetMSB(tp_first[adr_ptcl_tmp].adr_ptcl_) == 0) {
            ...
            // if(sp_first[adr_ptcl_tmp].mass == 0.0) { std::cerr<<"check
            // a"<<std::endl; } // another node sent sp with mass of 0
            sp_list.pushBackNoCheck(sp_first[adr_ptcl_tmp++]);
          }
        }
      }
...
template <class Ttc, class Ttp, class Tep, class Tsp>
void MakeInteractionListLongCutoffEPSP(
    const ReallocatableArray<Ttc> &tc_first, const S32 adr_tc,
    const ReallocatableArray<Ttp> &tp_first,
    const ReallocatableArray<Tep> &ep_first, ReallocatableArray<Tep> &ep_list,
    const ReallocatableArray<Tsp> &sp_first, ReallocatableArray<Tsp> &sp_list,
    const F64ort &cell_box, const F64ort &pos_target_box, const F64 r_crit_sq,
    const F64 r_cut_sq, const S32 n_leaf_limit) {
  U32 open_bits = 0;
  for (S32 i = 0; i < N_CHILDREN; i++) {
    const F64vec pos_tmp = tc_first[adr_tc + i].mom_.getPos();
    open_bits |= ((pos_target_box.getDistanceMinSQ(pos_tmp) <= r_crit_sq) << i);
    ...
    if (n_child == 0)
      ...
      if (!(tc_child->isLeaf(n_leaf_limit))) {
        ...
        MakeInteractionListLongCutoffEPSP<Ttc, Ttp, Tep, Tsp>(
            tc_first, tc_first[adr_tc_child].adr_tc_, tp_first, ep_first,
            ep_list, sp_first, sp_list, child_cell_box, pos_target_box,
            r_crit_sq * 0.25, r_cut_sq, n_leaf_limit);
      } else {
        S32 adr_ptcl_tmp = tc_child->adr_ptcl_;
...
void MakeInteractionListLongCutoffEPForZeroTheta(
    const ReallocatableArray<Ttc> &tc_first, const S32 adr_tc,
    const ReallocatableArray<Ttp> &tp_first,
    const ReallocatableArray<Tep> &ep_first, ReallocatableArray<Tep> &ep_list,
    const F64ort &cell_box, const F64ort &pos_target_box, const F64 r_cut_sq,
    const S32 n_leaf_limit) {
  U32 open_bits = 0;
  for (S32 i = 0; i < N_CHILDREN; i++) {
#if 0
    ...
    if (n_child == 0)
      ...
      if (!(tc_child->isLeaf(n_leaf_limit))) {
        ...
        MakeInteractionListLongCutoffEPForZeroTheta<Ttc, Ttp, Tep>(
            tc_first, tc_first[adr_tc_child].adr_tc_, tp_first, ep_first,
            ep_list, child_cell_box, pos_target_box, r_cut_sq, n_leaf_limit);
      } else {
        S32 adr_ptcl_tmp = tc_child->adr_ptcl_;
        ep_list.reserveEmptyAreaAtLeast(n_child);
        for (S32 ip = 0; ip < n_child; ip++) {
...
inline void MakeListUsingOuterBoundary(
    const Ttc *tc_first, const S32 adr_tc, const Tep2 *ep_first,
    ReallocatableArray<Tep3> &ep_list,
    const F64ort &pos_target_box,  // position of domain
    const S32 n_leaf_limit, const F64vec &shift = F64vec(0.0)) {
  U32 open_bits = 0;
  for (S32 i = 0; i < N_CHILDREN; i++) {
    // open_bits |= (pos_target_box.contained(
    // tc_first[adr_tc+i].mom_.getVertexOut() ) << i);
    open_bits |=
    ...
    if ((open_bits >> i) & 0x1) {
      ...
      if (!(tc_child->isLeaf(n_leaf_limit))) {
        ...
        for (S32 ip = 0; ip < n_child; ip++, adr_ptcl_tmp++) {
          ...
          if (dis_sq_tmp > size_tmp * size_tmp) continue;
#endif
          if (pos_root_cell.notContained(pos_tmp + shift))
            continue;  // add M.I. 2016/03/12
          ep_list.increaseSize();
          ep_list.back() = ep_first[adr_ptcl_tmp];
          const F64vec pos_new = ep_list.back().getPos() + shift;
          ep_list.back().setPos(pos_new);
...
template <class Ttc, class Tep2, class Tep3>
inline void MakeListUsingOuterBoundaryAndInnerBoundary(
    const Ttc *tc_first, const S32 adr_tc, const Tep2 *ep_first,
    ReallocatableArray<Tep3> &ep_list,
  ...
  const _fjsp_v2r8 xi_h_in =
      _fjsp_set_v2r8(pos_target_box_in.high_.x, pos_target_box_in.high_.x);
  const _fjsp_v2r8 xi_l_in =
      _fjsp_set_v2r8(pos_target_box_in.low_.x, pos_target_box_in.low_.x);
  const _fjsp_v2r8 yi_h_in =
      _fjsp_set_v2r8(pos_target_box_in.high_.y, pos_target_box_in.high_.y);
  const _fjsp_v2r8 yi_l_in =
      _fjsp_set_v2r8(pos_target_box_in.low_.y, pos_target_box_in.low_.y);
  const _fjsp_v2r8 zi_h_in =
      _fjsp_set_v2r8(pos_target_box_in.high_.z, pos_target_box_in.high_.z);
  const _fjsp_v2r8 zi_l_in =
      _fjsp_set_v2r8(pos_target_box_in.low_.z, pos_target_box_in.low_.z);

  const _fjsp_v2r8 xi_h_out =
      _fjsp_set_v2r8(pos_target_box_out.high_.x, pos_target_box_out.high_.x);
  const _fjsp_v2r8 xi_l_out =
      _fjsp_set_v2r8(pos_target_box_out.low_.x, pos_target_box_out.low_.x);
  const _fjsp_v2r8 yi_h_out =
      _fjsp_set_v2r8(pos_target_box_out.high_.y, pos_target_box_out.high_.y);
  const _fjsp_v2r8 yi_l_out =
      _fjsp_set_v2r8(pos_target_box_out.low_.y, pos_target_box_out.low_.y);
  const _fjsp_v2r8 zi_h_out =
      _fjsp_set_v2r8(pos_target_box_out.high_.z, pos_target_box_out.high_.z);
  const _fjsp_v2r8 zi_l_out =
      _fjsp_set_v2r8(pos_target_box_out.low_.z, pos_target_box_out.low_.z);
  for (S32 i = 0; i < N_CHILDREN / 4; i++) {
    ...
    const _fjsp_v2r8 xj_h_in_a =
        _fjsp_set_v2r8(mom0_in.high_.x, mom1_in.high_.x);
    const _fjsp_v2r8 xj_l_in_a = _fjsp_set_v2r8(mom0_in.low_.x, mom1_in.low_.x);
    const _fjsp_v2r8 xj_h_in_b =
        _fjsp_set_v2r8(mom2_in.high_.x, mom3_in.high_.x);
    const _fjsp_v2r8 xj_l_in_b = _fjsp_set_v2r8(mom2_in.low_.x, mom3_in.low_.x);
    const _fjsp_v2r8 xj_h_out_a =
        _fjsp_set_v2r8(mom0_out.high_.x, mom1_out.high_.x);
    const _fjsp_v2r8 xj_l_out_a =
        _fjsp_set_v2r8(mom0_out.low_.x, mom1_out.low_.x);
    const _fjsp_v2r8 xj_h_out_b =
        _fjsp_set_v2r8(mom2_out.high_.x, mom3_out.high_.x);
    const _fjsp_v2r8 xj_l_out_b =
        _fjsp_set_v2r8(mom2_out.low_.x, mom3_out.low_.x);

    const _fjsp_v2r8 yj_h_in_a =
        _fjsp_set_v2r8(mom0_in.high_.y, mom1_in.high_.y);
    const _fjsp_v2r8 yj_l_in_a = _fjsp_set_v2r8(mom0_in.low_.y, mom1_in.low_.y);
    const _fjsp_v2r8 yj_h_in_b =
        _fjsp_set_v2r8(mom2_in.high_.y, mom3_in.high_.y);
    const _fjsp_v2r8 yj_l_in_b = _fjsp_set_v2r8(mom2_in.low_.y, mom3_in.low_.y);
    const _fjsp_v2r8 yj_h_out_a =
        _fjsp_set_v2r8(mom0_out.high_.y, mom1_out.high_.y);
    const _fjsp_v2r8 yj_l_out_a =
        _fjsp_set_v2r8(mom0_out.low_.y, mom1_out.low_.y);
    const _fjsp_v2r8 yj_h_out_b =
        _fjsp_set_v2r8(mom2_out.high_.y, mom3_out.high_.y);
    const _fjsp_v2r8 yj_l_out_b =
        _fjsp_set_v2r8(mom2_out.low_.y, mom3_out.low_.y);

    const _fjsp_v2r8 zj_h_in_a =
        _fjsp_set_v2r8(mom0_in.high_.z, mom1_in.high_.z);
    const _fjsp_v2r8 zj_l_in_a = _fjsp_set_v2r8(mom0_in.low_.z, mom1_in.low_.z);
    const _fjsp_v2r8 zj_h_in_b =
        _fjsp_set_v2r8(mom2_in.high_.z, mom3_in.high_.z);
    const _fjsp_v2r8 zj_l_in_b = _fjsp_set_v2r8(mom2_in.low_.z, mom3_in.low_.z);
    const _fjsp_v2r8 zj_h_out_a =
        _fjsp_set_v2r8(mom0_out.high_.z, mom1_out.high_.z);
    const _fjsp_v2r8 zj_l_out_a =
        _fjsp_set_v2r8(mom0_out.low_.z, mom1_out.low_.z);
    const _fjsp_v2r8 zj_h_out_b =
        _fjsp_set_v2r8(mom2_out.high_.z, mom3_out.high_.z);
    const _fjsp_v2r8 zj_l_out_b =
        _fjsp_set_v2r8(mom2_out.low_.z, mom3_out.low_.z);

    const _fjsp_v2r8 mask_x_io_ji_a =
        _fjsp_and_v2r8(_fjsp_cmple_v2r8(xi_l_out, xj_h_in_a),
                       _fjsp_cmple_v2r8(xj_l_in_a, xi_h_out));  // i:out, j:in
    const _fjsp_v2r8 mask_x_io_ji_b =
        _fjsp_and_v2r8(_fjsp_cmple_v2r8(xi_l_out, xj_h_in_b),
                       _fjsp_cmple_v2r8(xj_l_in_b, xi_h_out));
    const _fjsp_v2r8 mask_x_ii_jo_a =
        _fjsp_and_v2r8(_fjsp_cmple_v2r8(xi_l_in, xj_h_out_a),
                       _fjsp_cmple_v2r8(xj_l_out_a, xi_h_in));  // i:in, j:out
    const _fjsp_v2r8 mask_x_ii_jo_b =
        _fjsp_and_v2r8(_fjsp_cmple_v2r8(xi_l_in, xj_h_out_b),
                       _fjsp_cmple_v2r8(xj_l_out_b, xi_h_in));

    const _fjsp_v2r8 mask_y_io_ji_a =
        _fjsp_and_v2r8(_fjsp_cmple_v2r8(yi_l_out, yj_h_in_a),
                       _fjsp_cmple_v2r8(yj_l_in_a, yi_h_out));  // i:out, j:in
    const _fjsp_v2r8 mask_y_io_ji_b =
        _fjsp_and_v2r8(_fjsp_cmple_v2r8(yi_l_out, yj_h_in_b),
                       _fjsp_cmple_v2r8(yj_l_in_b, yi_h_out));
    const _fjsp_v2r8 mask_y_ii_jo_a =
        _fjsp_and_v2r8(_fjsp_cmple_v2r8(yi_l_in, yj_h_out_a),
                       _fjsp_cmple_v2r8(yj_l_out_a, yi_h_in));  // i:in, j:out
    const _fjsp_v2r8 mask_y_ii_jo_b =
        _fjsp_and_v2r8(_fjsp_cmple_v2r8(yi_l_in, yj_h_out_b),
                       _fjsp_cmple_v2r8(yj_l_out_b, yi_h_in));

    const _fjsp_v2r8 mask_z_io_ji_a =
        _fjsp_and_v2r8(_fjsp_cmple_v2r8(zi_l_out, zj_h_in_a),
                       _fjsp_cmple_v2r8(zj_l_in_a, zi_h_out));  // i:out, j:in
    const _fjsp_v2r8 mask_z_io_ji_b =
        _fjsp_and_v2r8(_fjsp_cmple_v2r8(zi_l_out, zj_h_in_b),
                       _fjsp_cmple_v2r8(zj_l_in_b, zi_h_out));
    const _fjsp_v2r8 mask_z_ii_jo_a =
        _fjsp_and_v2r8(_fjsp_cmple_v2r8(zi_l_in, zj_h_out_a),
                       _fjsp_cmple_v2r8(zj_l_out_a, zi_h_in));  // i:in, j:out
    const _fjsp_v2r8 mask_z_ii_jo_b =
        _fjsp_and_v2r8(_fjsp_cmple_v2r8(zi_l_in, zj_h_out_b),
                       _fjsp_cmple_v2r8(zj_l_out_b, zi_h_in));

    const _fjsp_v2r8 mask_tmp_a = _fjsp_and_v2r8(
        _fjsp_or_v2r8(
            _fjsp_and_v2r8(mask_x_io_ji_a,
                           _fjsp_and_v2r8(mask_y_io_ji_a, mask_z_io_ji_a)),
            _fjsp_and_v2r8(mask_x_ii_jo_a,
                           _fjsp_and_v2r8(mask_y_ii_jo_a, mask_z_ii_jo_a))),
        ONE);

    const _fjsp_v2r8 mask_tmp_b = _fjsp_and_v2r8(
        _fjsp_or_v2r8(
            _fjsp_and_v2r8(mask_x_io_ji_b,
                           _fjsp_and_v2r8(mask_y_io_ji_b, mask_z_io_ji_b)),
            _fjsp_and_v2r8(mask_x_ii_jo_b,
                           _fjsp_and_v2r8(mask_y_ii_jo_b, mask_z_ii_jo_b))),
        ONE);
    ...
    if ((open_bits >> i) & 0x1) {
      ...
      if (!(tc_child[i].isLeaf(n_leaf_limit))) {
        ...
        for (S32 ip = 0; ip < n_child; ip++, adr_ptcl_tmp++) {
          ...
          if (pos_target_box_out.notContained(pos_tmp) &&
              dis_sq_tmp > size_tmp * size_tmp)
            continue;
          ep_list.increaseSize();
          ep_list.back() = ep_first[adr_ptcl_tmp];
          const F64vec pos_new = ep_list.back().getPos() + shift;
...
template <class Ttc, class Tep2, class Tep3>
inline void MakeListUsingInnerBoundary(
    const Ttc *tc_first, const S32 adr_tc, const Tep2 *ep_first,
    ReallocatableArray<Tep3> &ep_list,
    const F64ort &pos_target_box,  // position of domain
    const S32 n_leaf_limit, const F64vec &shift = F64vec(0.0)) {
  ...
  static const _fjsp_v2r8 ONE = _fjsp_set_v2r8(1.0, 1.0);
  F64 open_mask[N_CHILDREN];
  const _fjsp_v2r8 xi_h =
      _fjsp_set_v2r8(pos_target_box.high_.x, pos_target_box.high_.x);
  const _fjsp_v2r8 xi_l =
      _fjsp_set_v2r8(pos_target_box.low_.x, pos_target_box.low_.x);
  const _fjsp_v2r8 yi_h =
      _fjsp_set_v2r8(pos_target_box.high_.y, pos_target_box.high_.y);
  const _fjsp_v2r8 yi_l =
      _fjsp_set_v2r8(pos_target_box.low_.y, pos_target_box.low_.y);
  const _fjsp_v2r8 zi_h =
      _fjsp_set_v2r8(pos_target_box.high_.z, pos_target_box.high_.z);
  const _fjsp_v2r8 zi_l =
      _fjsp_set_v2r8(pos_target_box.low_.z, pos_target_box.low_.z);
  for (S32 i = 0; i < N_CHILDREN / 4; i++) {
    const S32 i0 = i * 4;
    ...
    const _fjsp_v2r8 mask_x = _fjsp_and_v2r8(_fjsp_cmple_v2r8(xi_l, xj_h),
                                             _fjsp_cmple_v2r8(xj_l, xi_h));
    const _fjsp_v2r8 mask_x_b = _fjsp_and_v2r8(_fjsp_cmple_v2r8(xi_l, xj_h_b),
                                               _fjsp_cmple_v2r8(xj_l_b, xi_h));
    ...
    const _fjsp_v2r8 mask_y = _fjsp_and_v2r8(_fjsp_cmple_v2r8(yi_l, yj_h),
                                             _fjsp_cmple_v2r8(yj_l, yi_h));
    const _fjsp_v2r8 mask_y_b = _fjsp_and_v2r8(_fjsp_cmple_v2r8(yi_l, yj_h_b),
                                               _fjsp_cmple_v2r8(yj_l_b, yi_h));
    ...
    const _fjsp_v2r8 mask_z = _fjsp_and_v2r8(_fjsp_cmple_v2r8(zi_l, zj_h),
                                             _fjsp_cmple_v2r8(zj_l, zi_h));
    const _fjsp_v2r8 mask_z_b = _fjsp_and_v2r8(_fjsp_cmple_v2r8(zi_l, zj_h_b),
                                               _fjsp_cmple_v2r8(zj_l_b, zi_h));

    // 1;open(go deeper), 0:not open
    const _fjsp_v2r8 mask_tmp_a = _fjsp_and_v2r8(
        _fjsp_and_v2r8(mask_x, _fjsp_and_v2r8(mask_y, mask_z)), ONE);
    const _fjsp_v2r8 mask_tmp_b = _fjsp_and_v2r8(
        _fjsp_and_v2r8(mask_x_b, _fjsp_and_v2r8(mask_y_b, mask_z_b)), ONE);
    _fjsp_storeh_v2r8(open_mask + i0, mask_tmp_a);
    _fjsp_storel_v2r8(open_mask + i1, mask_tmp_a);
    _fjsp_storeh_v2r8(open_mask + i2, mask_tmp_b);
    _fjsp_storel_v2r8(open_mask + i3, mask_tmp_b);
  ...
  for (S32 i = 0; i < N_CHILDREN; i++) {
    open_bits |=
        (pos_target_box.contained(tc_first[adr_tc + i].mom_.getVertexIn())
         << i);
  }
#endif  // FAST_WALK_K
  for (S32 i = 0; i < N_CHILDREN; i++) {
    ...
    if ((open_bits >> i) & 0x1) {
      ...
      if (!(tc_child->isLeaf(n_leaf_limit))) {
        ...
        for (S32 ip = 0; ip < n_child; ip++) {
          ep_list.pushBackNoCheck(ep_first[adr_ptcl_tmp]);
          const F64vec pos_new =
              ep_first[adr_ptcl_tmp++].getPos() + shift;  // for periodic mode
          ep_list.back().setPos(pos_new);
        }
      }
    }
...
inline void IsOverlappedUsingOuterBoundaryOfTreeImpl(const Ttc *tc_first,
  ...
  for (S32 i = 0; i < N_CHILDREN; i++) {
    open_bits |=
        (tc_first[adr_tc + i].mom_.getVertexOut().contained(pos_box) << i);
  }
  ...
  for (S32 i = 0; i < N_CHILDREN; i++) {
    ...
    if ((open_bits >> i) & 0x1) {
      if (!(tc_child->isLeaf(n_leaf_limit))) {
        IsOverlappedUsingOuterBoundaryOfTreeImpl(
            tc_first, tc_first[adr_tc_child].adr_tc_, pos_box, n_leaf_limit,
            is_overlapped);
      } else {
        // leaf and overlapped
        is_overlapped = 1;
...
inline void MakeLETListByDoubleWalkImpl(
    const Ttca *tc_first_A, const Ttcb *tc_first_B, const S32 adr_tc_B,
    const Tepb *ep_first_B, const F64ort &pos_domain, const S32 n_leaf_limit_A,
    const S32 n_leaf_limit_B, ReallocatableArray<S32> &id_ptcl_send) {
  U32 open_bits = 0;
  const U32 ONE = 1;
  // asm("# MakeLETListByDoubleWalkImpl");
  for (S32 i = 0; i < N_CHILDREN; i++) {
    if (tc_first_B[adr_tc_B + i].mom_.getVertexOut().contained(pos_domain) ||
        IsOverlappedUsingOuterBoundaryOfTree(
            tc_first_A, tc_first_B[adr_tc_B + i].mom_.getVertexIn(),
            n_leaf_limit_A)) {
      open_bits |= ONE << i;
    }
    /*
                open_bits |=
       (tc_first_B[adr_tc_B+i].mom_.getVertexOut().contained(pos_domain) << i);
                if( (~(open_bits>>i)) & 0x1 ){
                    open_bits |=
       IsOverlappedUsingOuterBoundaryOfTree(tc_first_A,
       tc_first_B[adr_tc_B+i].mom_.getVertexIn(), n_leaf_limit_A) << i;
                    //open_bits |=
       IsOverlappedUsingOuterBoundaryOfTree(tc_first_A,
       tc_first_B[adr_tc_B+i].mom_.getVertexOut(), n_leaf_limit_A) << i;
                    //open_bits |=
       tc_first_A->mom_.getVertexOut().contained(tc_first_B[adr_tc_B+i].mom_.getVertexIn())
       << i; // out
                    //open_bits |=
       tc_first_A->mom_.getVertexOut().contained(tc_first_B[adr_tc_B+i].mom_.getVertexOut())
       << i; // OK
                    //open_bits |=
       tc_first_B[adr_tc_B+i].mom_.getVertexIn().contained(tc_first_A->mom_.getVertexOut())
       << i; // out
                }
    */
  }
  ...
  for (S32 i = 0; i < N_CHILDREN; i++) {
    if ((open_bits >> i) & 0x1) {
      ...
      if (n_child == 0)
        ...
        MakeLETListByDoubleWalkImpl(tc_first_A, tc_first_B,
                                    tc_first_B[adr_tc_child].adr_tc_,
                                    ep_first_B, pos_domain, n_leaf_limit_A,
                                    n_leaf_limit_B, id_ptcl_send);
      } else {
        S32 adr_ptcl_tmp = tc_child->adr_ptcl_;
        for (S32 ip = 0; ip < n_child; ip++, adr_ptcl_tmp++) {
#ifdef ORIGINAL_SCATTER_MODE
          ...
          const F64 len_sq = ep_first_B[adr_ptcl_tmp].getRSearch() *
                             ep_first_B[adr_ptcl_tmp].getRSearch();
...
#endif
          // if(
          // pos_domain.contained(tc_first_B[adr_tc_child].mom_.getVertexOut()) )
          // continue;
...
template <class Ttca, class Ttcb, class Tepb>
inline void MakeLETListByDoubleWalk(
    const Ttca *tc_first_A, const Ttcb *tc_first_B, const Tepb *ep_first_B,
    const F64ort &pos_domain, const S32 n_leaf_limit_A,
    const S32 n_leaf_limit_B, ReallocatableArray<S32> &id_ptcl_send) {
  const S32 adr_tc_B = N_CHILDREN;
  if (!tc_first_B->isLeaf(n_leaf_limit_B)) {
    MakeLETListByDoubleWalkImpl(tc_first_A, tc_first_B, adr_tc_B, ep_first_B,
                                pos_domain, n_leaf_limit_A, n_leaf_limit_B,
                                id_ptcl_send);
  } else {
    if (tc_first_B->mom_.getVertexOut().contained(pos_domain) ||
        IsOverlappedUsingOuterBoundaryOfTree(
            tc_first_A, tc_first_B->mom_.getVertexIn(), n_leaf_limit_A)) {
...
inline void SearchNeighborListOneParticleScatter(
    const Tvec &pos_target, const Ttc *tc_first, const Ttp *tp_first,
    const S32 adr_tc, const ReallocatableArray<Tep> &ep_first,
    ReallocatableArray<Tep> &ep_list, const S32 n_leaf_limit) {
#if 1
  if (tc_first[adr_tc].isLeaf(n_leaf_limit)) {
    const S32 n_tmp = tc_first[adr_tc].n_ptcl_;
    U32 adr_tp_tmp = tc_first[adr_tc].adr_ptcl_;
  ...
  if (tc_first[adr_tc].isLeaf(n_leaf_limit)) {
    ...
    for (S32 i = 0; i < N_CHILDREN; i++) {
      ...
      if (n_child == 0)
        ...
        SearchNeighborListOneParticleScatter(pos_target, tc_first, tp_first,
                                             adr_tc_child + i, ep_first,
                                             ep_list, n_leaf_limit);
      }
    }
  }
  ...
  /*
      ...
      U32 open_bits = 0;
      for(S32 i=0; i<N_CHILDREN; i++){
          const F64ort vertex_cell =
  tc_first[adr_tc_child+i].mom_.getVertexOut(); open_bits |=
  vertex_cell.overlapped(pos_target) << i;
      }
      for(S32 i=0; i<N_CHILDREN; i++){
          const S32 n_child = tc_first[adr_tc_child+i].n_ptcl_;
          if( n_child == 0 ) continue;
...
template <class Tvec, class Ttc, class Ttp, class Tep>
inline void SearchNeighborListOneParticleGather(
    const Tvec &pos_target, const F64 &r_search_sq, const Ttc *tc_first,
    const Ttp *tp_first, const S32 adr_tc,
    const ReallocatableArray<Tep> &ep_first, ReallocatableArray<Tep> &ep_list,
    const S32 n_leaf_limit) {
#if 1
  if (tc_first[adr_tc].isLeaf(n_leaf_limit)) {
  ...
  if (tc_first[adr_tc].isLeaf(n_leaf_limit)) {
    ...
    for (S32 i = 0; i < N_CHILDREN; i++) {
      ...
      open_bits |= (vertex_cell.getDistanceMinSQ(pos_target) <= r_search_sq)
                   << i;
    }
    for (S32 i = 0; i < N_CHILDREN; i++) {
      const S32 n_child = tc_first[adr_tc_child + i].n_ptcl_;
      if (n_child == 0)
        ...
        SearchNeighborListOneParticleGather(pos_target, r_search_sq, tc_first,
                                            tp_first, adr_tc_child + i,
                                            ep_first, ep_list, n_leaf_limit);
      }
    }
  }
...
template <class Tvec, class Ttc, class Ttp, class Tep>
inline void SearchNeighborListOneParticleSymmetry(
    const Tvec &pos_target, const F64 &r_search_i_sq, const Ttc *tc_first,
    const Ttp *tp_first, const S32 adr_tc,
    const ReallocatableArray<Tep> &ep_first, ReallocatableArray<Tep> &ep_list,
    const S32 n_leaf_limit) {
#if 1
  if (tc_first[adr_tc].isLeaf(n_leaf_limit)) {
    ...
    for (S32 ip = 0; ip < n_tmp; ip++, adr_tp_tmp++) {
      U32 adr_ep_tmp = tp_first[adr_tp_tmp].adr_ptcl_;
      if (GetMSB(adr_ep_tmp) == 1) continue;
      const F64 r_search_j_sq =
          ep_first[adr_ep_tmp].getRSearch() * ep_first[adr_ep_tmp].getRSearch();
      const F64 r_search_sq =
          (r_search_i_sq > r_search_j_sq) ? r_search_i_sq : r_search_j_sq;
      const F64vec dr = ep_first[adr_ep_tmp].getPos() - pos_target;
      if (dr * dr <= r_search_sq) {
        ep_list.push_back(ep_first[adr_ep_tmp]);
      }
  ...
  if (tc_first[adr_tc].isLeaf(n_leaf_limit)) {
    ...
    S32 adr_ptcl_tmp = tc_first[adr_tc].adr_ptcl_;
    for (S32 ip = 0; ip < n_tmp; ip++, adr_ptcl_tmp++) {
      const F64 r_search_j_sq = ep_first[adr_ptcl_tmp].getRSearch() *
                                ep_first[adr_ptcl_tmp].getRSearch();
      const F64 r_search_sq =
          (r_search_i_sq > r_search_j_sq) ? r_search_i_sq : r_search_j_sq;
      const F64vec dr = ep_first[adr_ptcl_tmp].getPos() - pos_target;
      if (GetMSB(tp_first[adr_ptcl_tmp].adr_ptcl_) == 0 &&
    ...
    U32 open_bits = 0;
    for (S32 i = 0; i < N_CHILDREN; i++) {
      const F64ort vertex_cell_in =
          tc_first[adr_tc_child + i].mom_.getVertexIn();
      const F64ort vertex_cell_out =
          tc_first[adr_tc_child + i].mom_.getVertexOut();
      open_bits |=
          ((vertex_cell_in.getDistanceMinSQ(pos_target) <= r_search_i_sq) ||
           vertex_cell_out.overlapped(pos_target))
          << i;
    }
    for (S32 i = 0; i < N_CHILDREN; i++) {
      const S32 n_child = tc_first[adr_tc_child + i].n_ptcl_;
  ...
  while (!adr_tc_stack.empty()) {
    ...
    if (tc_first[adr_tc_curr].isLeaf(n_leaf_limit)) {
      ...
      for (S32 i = 0; i < N_CHILDREN; i++, adr_tc_child++) {
        if (pos_target_box_out.contained(
                tc_first[adr_tc_child].mom_.getVertexIn()) ||
            pos_target_box_in.contained(
                tc_first[adr_tc_child].mom_.getVertexOut())) {
...
inline CountT MakeInteractionListLongEPSPIteration(
  ...
  while (!adr_tc_stack.empty()) {
    ...
    for (S32 i = 0; i < N_CHILDREN; i++) {
      ...
      open_bits |=
          ((pos_target_box.getDistanceMinSQ(pos_tmp) <= r_crit_sq_curr) << i);
    }
#endif
    for (S32 i = 0; i < N_CHILDREN; i++) {
...
inline CountT MakeInteractionListLongEPSPIteration(
  ...
  while (!adr_tc_stack.empty()) {
    ...
    if (tc_first[adr_tc_curr].isLeaf(n_leaf_limit)) {
      ...
      for (S32 i = 0; i < N_CHILDREN; i++, adr_tc_child++) {
        if (tc_first[adr_tc_child].n_ptcl_ == 0)
          continue;
