...
class Timer {
 ...
 public:
  ...
  void dump(std::ostream& fout) {
    ...
    Comm::getMaxValue(total_time_per_loop, Comm::getRank(),
                      max_total_time_per_loop, max_rank_total_time_per_loop);

    double max_total_time_sub;
    int max_rank_total_time_sub;
    Comm::getMaxValue(total_time_sub_, Comm::getRank(), max_total_time_sub,
                      max_rank_total_time_sub);
    fout << "total_time= " << total_time_
         << "   total_time_per_loop(ave)= " << max_total_time_sub / n_trial_
         << " (@" << max_rank_total_time_per_loop
         << ")   max_total_time_per_loop= " << max_total_time_per_loop << " (@"
         << max_rank_total_time_per_loop << ")" << std::endl;
    ...
    for (size_t i = 0; i < cnt_; i++) {
      ...
      int max_rank_cum = 0;
      Comm::getMaxValue(cumulative_time, Comm::getRank(), max_time_cum,
                        max_rank_cum);

      split_time_total_[i] += split_time_[i];
      double max_time_split_total = 0.0;
      int max_rank_split_total = 0;
      Comm::getMaxValue(split_time_total_[i], Comm::getRank(),
                        max_time_split_total, max_rank_split_total);
      if (func_name_[i][0] == '\0') {
        fout << "split_time_[" << i << "]= " << split_time_[i]
             << "   cum_time= " << cumulative_time << "   fraction: "
             << ((double)split_time_[i]) / total_time_per_loop
             << "   max_time= " << max_time << " (@" << max_rank
             << ")   max_time_cum= " << max_time_cum << " (@" << max_rank_cum
             << ")  "
             << "   max_time_split_ave= " << max_time_split_total / n_trial_
             << " (@" << max_rank_split_total << ")" << std::endl;
      } else {
        fout << func_name_[i] << ": " << split_time_[i]
             << "   cum_time= " << cumulative_time << "   fraction: "
             << ((double)split_time_[i]) / total_time_per_loop
             << "   max_time= " << max_time << " (@" << max_rank
             << ")   max_time_cum= " << max_time_cum << " (@" << max_rank_cum
