...
template <class Ttc>
inline F64ort GetOuterBoundaryOfMyTree(
    TagSearchLong, const ReallocatableArray<Ttc> &tc_first) {
  return F64ort(-1234.5, -9876.5);
}

template <class Ttc>
inline F64ort GetOuterBoundaryOfMyTree(
    TagSearchLongCutoff, const ReallocatableArray<Ttc> &tc_first) {
  return tc_first[0].mom_.getVertexOut();
}

template <class Ttc>
inline F64ort GetOuterBoundaryOfMyTree(
    TagSearchLongScatter, const ReallocatableArray<Ttc> &tc_first) {
  return tc_first[0].mom_.getVertexOut();
}

template <class Ttc>
inline F64ort GetOuterBoundaryOfMyTree(
    TagSearchLongSymmetry, const ReallocatableArray<Ttc> &tc_first) {
  return tc_first[0].mom_.getVertexOut();
}

template <class Ttc>
inline F64ort GetOuterBoundaryOfMyTree(
    TagSearchShortGather, const ReallocatableArray<Ttc> &tc_first) {
  return tc_first[0].mom_.getVertexOut();
}

template <class Ttc>
inline F64ort GetOuterBoundaryOfMyTree(
    TagSearchShortSymmetry, const ReallocatableArray<Ttc> &tc_first) {
  return tc_first[0].mom_.getVertexOut();
}

template <class Ttc>
inline F64ort GetOuterBoundaryOfMyTree(
    TagSearchShortScatter, const ReallocatableArray<Ttc> &tc_first) {
  return tc_first[0].mom_.getVertexOut();
}
...
inline void CopyPtclFromTreeToSendBuf(
    TagSearchBoundaryConditionOpenOnly, ReallocatableArray<Tptcl> &ptcl_send,
    const ReallocatableArray<Ttc> &tc,
    const ReallocatableArray<S32> &adr_ptcl_send, const S32 n_ptcl,
    const S32 n_ptcl_offset, const F64vec &shift) {
...
template <class Tptcl, class Ttc>
inline void CopyPtclFromTreeToSendBuf(
    TagSearchBoundaryConditionOpenPeriodic,
    ReallocatableArray<Tptcl> &ptcl_send, const ReallocatableArray<Ttc> &tc,
    const ReallocatableArray<S32> &adr_ptcl_send, const S32 n_ptcl,
    const S32 n_ptcl_offset, const F64vec &shift) {
  for (S32 j = 0; j < n_ptcl; j++) {
    S32 adr = adr_ptcl_send[n_ptcl_offset + j];
...
inline void CopyPtclToSendBuf(TagSearchBoundaryConditionOpenOnly,
                              ...
                              const S32 n_ptcl, const S32 n_ptcl_offset,
                              const F64vec &shift) {
  for (S32 j = 0; j < n_ptcl; j++) {
    S32 adr = adr_ptcl_send[n_ptcl_offset + j];
...
inline void FindScatterParticle(const ReallocatableArray<Ttc> &tc_first,
                                ...
                                const DomainInfo &dinfo, const S32 n_leaf_limit,
  ...
  F64ort outer_boundary_of_my_tree =
      GetOuterBoundaryOfMyTree(typename TSM::search_type(), tc_first);
  S32 adr_tc = 0;
  S32 adr_tree_sp_first = 0;
  ExchangeOuterBoundary(typename TSM::search_type(), &outer_boundary_of_my_tree,
...
#pragma omp for schedule(dynamic, 4)
    ...
    for (S32 i = 0; i < n_proc; i++) {
      ...
      for (S32 j = n_image_tmp_prev; j < n_image_tmp; j++) {
        ...
        if (my_rank == i && j == n_image_tmp_prev) {
          n_ep_per_image_tmp[ith].push_back(adr_ep_send_tmp[ith].size() -
                                            n_ep_prev_2);  // is 0
          n_sp_per_image_tmp[ith].push_back(adr_sp_send_tmp[ith].size() -
                                            n_sp_prev_2);  // is 0
          continue;
        }
        F64ort pos_target_domain =
            dinfo.getPosDomain(i).shift(shift_per_image_tmp[ith][j]);
        TargetBox<TSM> target_box;
        SetTargetBoxExLet(
            target_box, pos_target_domain,
            outer_boundary_of_tree[i].shift(shift_per_image_tmp[ith][j]));
        MakeListUsingTreeRecursiveTop<TSM, Ttc, TreeParticle, Tep, Tsp,
                                      Twalkmode, TagChopLeafFalse,
                                      TagCopyInfoCloseNoSp>(
            tc_first, adr_tc, tp_first, ep_first, adr_ep_send_tmp[ith],
            sp_first, adr_sp_send_tmp[ith], target_box, r_crit_sq, n_leaf_limit,
  ...
  for (S32 i = 0; i < n_proc; i++) {
    n_disp_image_per_proc[i + 1] =
        n_disp_image_per_proc[i] + n_image_per_proc[i];
  }
...
#pragma omp parallel for
  ...
  ReallocatableArray<S32> n_disp_ep_per_image(n_image_tot + 1, n_image_tot + 1,
                                              1);
  ReallocatableArray<S32> n_disp_sp_per_image(n_image_tot + 1, n_image_tot + 1,
                                              1);
...
#pragma omp for schedule(dynamic, 4)
    ...
    for (S32 i = 0; i < n_proc; i++) {
      ...
      for (S32 j = n_image_tmp_prev; j < n_image_tmp; j++) {
        ...
        MakeListUsingTreeRecursiveTop<SEARCH_MODE_SCATTER, Ttc, TreeParticle,
                                      Tep, Tsp, Twalkmode, TagChopLeafFalse,
                                      TagCopyInfoCloseNoSp>(
            tc_first, adr_tc, tp_first, ep_first, adr_ep_send_tmp[ith],
            sp_first, adr_sp_send_tmp, target_box, r_crit_sq, n_leaf_limit,
            adr_tree_sp_first, F64vec(0.0));
...
inline void ExchangeLet(
    const ReallocatableArray<Tep> &ep, const ReallocatableArray<S32> &n_ep_send,
    const ReallocatableArray<S32> &n_ep_recv,
    const ReallocatableArray<S32> &n_ep_per_image,
    const ReallocatableArray<S32> &adr_ep_send, ReallocatableArray<Tep> &ep_org,
    const S32 n_ep_offset,  // ep_org[n_ep_offset] = n_ep_recv[0]
    const ReallocatableArray<Ttc> &tc, const ReallocatableArray<S32> &n_sp_send,
  ...
  n_disp_ep_send[0] = n_disp_sp_send[0] = n_disp_ep_recv[0] =
      n_disp_sp_recv[0] = 0;
  for (S32 i = 0; i < n_proc; i++) {
...
#pragma omp parallel for schedule(dynamic, 4)
  ...
  for (S32 i = 0; i < n_image_tot; i++) {
    ...
    CopyPtclToSendBuf(typename TSM::search_boundary_type(), ep_send, ep,
                      adr_ep_send, n_ep, n_ep_offset, shift);
    ...
    CopyPtclFromTreeToSendBuf(typename TSM::search_boundary_type(), sp_send, tc,
                              adr_sp_send, n_sp, n_sp_offset, shift);
    /*
  ...
  Comm::allToAllV(ep_send.getPointer(), n_ep_send.getPointer(),
                  n_disp_ep_send.getPointer(), ep_org.getPointer(n_ep_offset),
                  n_ep_recv.getPointer(), n_disp_ep_recv.getPointer());
  Comm::allToAllV(sp_send.getPointer(), n_sp_send.getPointer(),
                  n_disp_sp_send.getPointer(), sp_org.getPointer(),
                  n_sp_recv.getPointer(), n_disp_sp_recv.getPointer());
}
...
inline void ExchangeLet(const ReallocatableArray<Tep> &ep,
                        ...
                        ReallocatableArray<Tep> &ep_org, const S32 n_ep_offset,
                        const ReallocatableArray<F64vec> &shift_image_domain,
                        const ReallocatableArray<S32> &n_image_per_proc) {
  const S32 n_proc = Comm::getNumberOfProc();
  ReallocatableArray<S32> n_disp_ep_send(n_proc + 1, n_proc + 1, 1);
...
#pragma omp parallel for schedule(dynamic, 4)
  ...
  Comm::allToAllV(ep_send.getPointer(), n_ep_send.getPointer(),
                  n_disp_ep_send.getPointer(), ep_recv.getPointer(),
                  n_ep_recv.getPointer(), n_disp_ep_recv.getPointer());
}
...
inline void FindExchangeParticleDoubleWalk(
    ...
    const DomainInfo &dinfo, const S32 n_leaf_limit_B,
    ...
    const F64vec &center_tree, const F64 &full_len_tree) {
  const S32 n_proc = Comm::getNumberOfProc();
  const S32 n_thread = Comm::getNumberOfThread();
...
#pragma omp for schedule(dynamic, 1)
    ...
    for (S32 ib = 0; ib < rank_src.size(); ib++) {
      ...
      for (S32 ip = adr_ptcl_head; ip < adr_ptcl_end; ip++) {
        ...
        const S32vec id_image_new =
            CalcIDOfImageDomain(pos_root_domain, pos_target, pa);
        if (id_image_old != id_image_new) {
          ...
          shift_per_image_tmp[ith].push_back(
              F64vec(id_image_new.x * len_root_domain.x,
                     id_image_new.y * len_root_domain.y));
#else
          shift_per_image_tmp[ith].push_back(
              F64vec(id_image_new.x * len_root_domain.x,
                     id_image_new.y * len_root_domain.y,
                     id_image_new.z * len_root_domain.z));
#endif
      ...
      for (S32 ii = adr_image_head; ii < adr_image_end; ii++) {
        ...
        for (S32 ip = adr_epj_src_per_image_tmp[ith][ii];
             ip < adr_epj_src_per_image_tmp[ith][ii] +
                      n_epj_src_per_image_tmp[ith][ii];
             ip++, n_cnt++) {
          tp_tmp[ith][n_cnt].setFromEP(epj_A[ip], ip);
          // if(Comm::getRank()==0) std::cerr<<"epj_A[ip].pos=
          // "<<epj_A[ip].pos<<std::endl;
        }

        std::sort(tp_tmp[ith].getPointer(), tp_tmp[ith].getPointer() + n_cnt,
                  LessOPKEY());
        ...
        CalcMomentST(adr_tc_level_partition_tmp[ith].getPointer(),
                     tc_tmp[ith].getPointer(), epj_sorted_tmp[ith].getPointer(),
                     lev_max_A, n_leaf_limit_A);

        const S32 n_epj_send_per_image_prev = adr_ptcl_send_tmp[ith].size();
        MakeListDoubleWalkTop(tc_tmp[ith], tc_first_B, epj_A, pos_domain,
                              n_leaf_limit_A, n_leaf_limit_B,
                              adr_ptcl_send_tmp[ith]);
        n_epj_send_per_image_tmp[ith].push_back(adr_ptcl_send_tmp[ith].size() -
                                                n_epj_send_per_image_prev);

      }  // end of for ii=0 to n_image
      n_epj_send_per_proc[rank_tmp] =
          adr_ptcl_send_tmp[ith].size() - n_epj_send_cum_prev;
    }  // end of OMP for
  }    // end of OMP scope
  ...
  for (S32 i = 0; i < n_proc; i++) {
    n_disp_image_send_per_proc[i + 1] =
        n_disp_image_send_per_proc[i] + n_image_send_per_proc[i];
  }
  ...
  for (S32 i = 0; i < n_image_send_tot; i++) {
    n_disp_epj_send_per_image[i + 1] =
        n_disp_epj_send_per_image[i] + n_epj_send_per_image[i];
  }
...
inline void SetLocalEssentialTreeToGlobalTreeImpl(
    ...
    const ReallocatableArray<Tspj> &spj_org, const S32 n_loc,
    ReallocatableArray<Ttp> &tp_glb, const bool flag_reuse = false) {
...
inline void SetLocalEssentialTreeToGlobalTreeImpl(
    const ReallocatableArray<Tepj> &epj_org, const S32 n_loc,
    ReallocatableArray<Ttp> &tp_glb, const bool flag_reuse = false) {
  const S32 n_loc_ep = epj_org.size();
...
template <class Ttc>
inline void SetOuterBoxGlobalTreeForLongCutoffRecursive(
    Ttc tc[], const S32 n_leaf_limit, const F64 r_cut, const S32 adr_tc,
    const F64 tc_hlen, const F64vec tc_cen) {
  ...
  for (S32 i = 0; i < N_CHILDREN; i++) {
    ...
    if (tc[adr_tc + i].n_ptcl_ <= 0)
      ...
      SetOuterBoxGlobalTreeForLongCutoffRecursive(
          tc, n_leaf_limit, r_cut, child_adr_tc, child_hlen, child_cen);
    }
  }
}

template <class Ttc, class Tepj>
inline void SetOuterBoxGlobalTreeForLongCutoffTop(TagSearchLongCutoff, Ttc tc[],
                                                  Tepj epj[],
...
template <class Ttc, class Tepj>
inline void SetOuterBoxGlobalTreeForLongCutoffTop(TagSearchLong, Ttc tc[],
                                                  Tepj epj[],
                                                  const S32 n_leaf_limit,
                                                  const F64 tc_hlen,
                                                  const F64vec tc_cen) {
