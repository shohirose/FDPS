...
class Comm {
 private:
  ...
  /*
      template<class Tfloat, class Tint>
      static void allreduceMin(const Tfloat & f_in, const Tint & i_in, Tfloat &
  f_out, Tint & i_out){ #ifdef PARTICLE_SIMULATOR_MPI_PARALLEL struct{ Tfloat x;
              Tint y;
          } loc, glb;
          loc.x = f_in;
          loc.y = i_in;
          MPI_Allreduce(&loc, &glb, 1, GetDataType<Tfloat, Tint>(), MPI_MINLOC,
  MPI_COMM_WORLD); f_out = glb.x; i_out = glb.y; #else f_out = f_in; i_out =
  i_in; #endif
      }
  */
  template <class Tfloat>
  static void allreduceMin(const Tfloat &f_in, const int &i_in, Tfloat &f_out,
                           int &i_out) {
#ifdef PARTICLE_SIMULATOR_MPI_PARALLEL
    struct {
      Tfloat x;
    ...
    loc.x = f_in;
    loc.y = i_in;
    MPI_Allreduce(&loc, &glb, 1, GetDataType<Tfloat, int>(), MPI_MINLOC,
                  MPI_COMM_WORLD);
    f_out = glb.x;
    i_out = glb.y;
#else
  ...
  /*
      template<class Tfloat, class Tint>
      static void allreduceMax(const Tfloat & f_in, const Tint & i_in, Tfloat &
  f_out, Tint & i_out){ #ifdef PARTICLE_SIMULATOR_MPI_PARALLEL struct{ Tfloat x;
              Tint y;
          } loc, glb;
          loc.x = f_in;
          loc.y = i_in;
          MPI_Allreduce(&loc, &glb, 1, GetDataType<Tfloat, Tint>(), MPI_MAXLOC,
  MPI_COMM_WORLD); f_out = glb.x; i_out = glb.y;
          //if(Comm::getRank() == 0){std::cout<<"glb.x="<<glb.x<<"
  glb.y="<<glb.y<<std::endl;} #else f_out = f_in; i_out = i_in; #endif
      }
  */
  template <class Tfloat>
  static void allreduceMax(const Tfloat &f_in, const int &i_in, Tfloat &f_out,
                           int &i_out) {
#ifdef PARTICLE_SIMULATOR_MPI_PARALLEL
    struct {
      Tfloat x;
    ...
    loc.x = f_in;
    loc.y = i_in;
    MPI_Allreduce(&loc, &glb, 1, GetDataType<Tfloat, int>(), MPI_MAXLOC,
                  MPI_COMM_WORLD);
    f_out = glb.x;
    i_out = glb.y;
    // if(Comm::getRank() == 0){std::cout<<"glb.x="<<glb.x<<"
    // glb.y="<<glb.y<<std::endl;}
#else
    f_out = f_in;
    i_out = i_in;
#endif
 ...
 public:
  ...
  // template<class Tfloat, class Tint> static inline void getMinValue(const
  // Tfloat & f_in, const Tint & i_in, Tfloat & f_out, Tint & i_out);
  // template<class Tfloat, class Tint> static inline void getMaxValue(const
  // Tfloat & f_in, const Tint & i_in, Tfloat & f_out, Tint & i_out);
  template <class Tfloat>
  static inline void getMinValue(const Tfloat &f_in, const int &i_in,
                                 Tfloat &f_out, int &i_out);
  template <class Tfloat>
  static inline void getMaxValue(const Tfloat &f_in, const int &i_in,
                                 Tfloat &f_out, int &i_out);
  template <class T>
...
static inline void Initialize(int &argc, char **&argv,
  ...
  if (Comm::getRank() == 0) {
    ...
    std::cerr << "     ||     Version 5.0g (2019/09)       ||" << std::endl;
    std::cerr << "     \\\\==================================//" << std::endl;
    std::cerr << "" << std::endl;
    std::cerr << "       Home   : https://github.com/fdps/fdps " << std::endl;
    std::cerr << "       E-mail : fdps-support@mail.jmlab.jp" << std::endl;
    std::cerr << "       Licence: MIT (see, "
                 "https://github.com/FDPS/FDPS/blob/master/LICENSE)"
              << std::endl;
    std::cerr << "       Note   : Please cite the following papers."
              << std::endl;
    std::cerr << "                - Iwasawa et al. (2016, Publications of the "
                 "Astronomical Society of Japan, 68, 54)"
              << std::endl;
    std::cerr << "                - Namekata et al. (2018, Publications of the "
                 "Astronomical Society of Japan, 70, 70)"
              << std::endl;
    std::cerr << "" << std::endl;
    std::cerr << "       Copyright (C) 2015 " << std::endl;
    std::cerr << "         Masaki Iwasawa, Ataru Tanikawa, Natsuki Hosono,"
              << std::endl;
    std::cerr
        << "         Keigo Nitadori, Takayuki Muranushi, Daisuke Namekata,"
        << std::endl;
    std::cerr << "         Kentaro Nomura, Junichiro Makino and many others"
              << std::endl;
#ifdef MONAR
    if (flag_monar) {
      std::cerr << "　　 ^__^　 ／‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾" << std::endl;
      std::cerr << "　　( ´∀｀)＜******** FDPS has successfully begun. ********"
                << std::endl;
      std::cerr
...
template <>
inline double Comm::getMinValue<double>(const double &val) {
  return allreduceMin(val);
}
template <>
inline int Comm::getMinValue<int>(const int &val) {
  return allreduceMin(val);
}
template <>
inline long Comm::getMinValue<long>(const long &val) {
  return allreduceMin(val);
}
template <>
inline long long Comm::getMinValue<long long>(const long long &val) {
  return allreduceMin(val);
}
template <>
inline unsigned int Comm::getMinValue<unsigned int>(const unsigned int &val) {
  return allreduceMin(val);
}
template <>
inline unsigned long Comm::getMinValue<unsigned long>(
    const unsigned long &val) {
  return allreduceMin(val);
}
template <>
inline unsigned long long Comm::getMinValue<unsigned long long>(
    const unsigned long long &val) {
  return allreduceMin(val);
}
template <>
inline Vector2<float> Comm::getMinValue<Vector2<float> >(
    const Vector2<float> &val) {
#ifdef PARTICLE_SIMULATOR_MPI_PARALLEL
  Vector2<float> ret;
...
template <>
inline Vector2<double> Comm::getMinValue<Vector2<double> >(
    const Vector2<double> &val) {
#ifdef PARTICLE_SIMULATOR_MPI_PARALLEL
  Vector2<double> ret;
...
template <>
inline Vector3<float> Comm::getMinValue<Vector3<float> >(
    const Vector3<float> &val) {
#ifdef PARTICLE_SIMULATOR_MPI_PARALLEL
  Vector3<float> ret;
...
template <>
inline Vector3<double> Comm::getMinValue<Vector3<double> >(
    const Vector3<double> &val) {
#ifdef PARTICLE_SIMULATOR_MPI_PARALLEL
  Vector3<double> ret;
...
template <>
inline float Comm::getMaxValue<float>(const float &val) {
  return allreduceMax(val);
}
template <>
inline double Comm::getMaxValue<double>(const double &val) {
  return allreduceMax(val);
}
template <>
inline int Comm::getMaxValue<int>(const int &val) {
  return allreduceMax(val);
}
template <>
inline long Comm::getMaxValue<long>(const long &val) {
  return allreduceMax(val);
}
template <>
inline long long Comm::getMaxValue<long long>(const long long &val) {
  return allreduceMax(val);
}
template <>
inline unsigned int Comm::getMaxValue<unsigned int>(const unsigned int &val) {
  return allreduceMax(val);
}
template <>
inline unsigned long Comm::getMaxValue<unsigned long>(
    const unsigned long &val) {
  return allreduceMax(val);
}
template <>
inline unsigned long long Comm::getMaxValue<unsigned long long>(
    const unsigned long long &val) {
  return allreduceMax(val);
}
template <>
inline Vector2<float> Comm::getMaxValue<Vector2<float> >(
    const Vector2<float> &val) {
#ifdef PARTICLE_SIMULATOR_MPI_PARALLEL
  Vector2<float> ret;
...
template <>
inline Vector2<double> Comm::getMaxValue<Vector2<double> >(
    const Vector2<double> &val) {
#ifdef PARTICLE_SIMULATOR_MPI_PARALLEL
  Vector2<double> ret;
...
template <>
inline Vector3<float> Comm::getMaxValue<Vector3<float> >(
    const Vector3<float> &val) {
#ifdef PARTICLE_SIMULATOR_MPI_PARALLEL
  Vector3<float> ret;
...
template <>
inline Vector3<double> Comm::getMaxValue<Vector3<double> >(
    const Vector3<double> &val) {
#ifdef PARTICLE_SIMULATOR_MPI_PARALLEL
  Vector3<double> ret;
...
template <>
inline float Comm::getSum(const float &val) {
  return allreduceSum(val);
}
template <>
inline double Comm::getSum(const double &val) {
  return allreduceSum(val);
}
template <>
inline int Comm::getSum(const int &val) {
  return allreduceSum(val);
}
template <>
inline unsigned int Comm::getSum(const unsigned int &val) {
  return allreduceSum(val);
}
template <>
inline long Comm::getSum(const long &val) {
  return allreduceSum(val);
}
template <>
inline unsigned long Comm::getSum(const unsigned long &val) {
  return allreduceSum(val);
}
template <>
inline long long int Comm::getSum(const long long int &val) {
  return allreduceSum(val);
}
template <>
inline unsigned long long int Comm::getSum(const unsigned long long int &val) {
  return allreduceSum(val);
}
template <>
inline Vector3<float> Comm::getSum(const Vector3<float> &val) {
#ifdef PARTICLE_SIMULATOR_MPI_PARALLEL
  Vector3<float> ret;
...
template <>
inline Vector3<double> Comm::getSum(const Vector3<double> &val) {
#ifdef PARTICLE_SIMULATOR_MPI_PARALLEL
  Vector3<double> ret;
...
template <>
inline Vector2<float> Comm::getSum(const Vector2<float> &val) {
#ifdef PARTICLE_SIMULATOR_MPI_PARALLEL
  Vector2<float> ret;
...
template <>
inline Vector2<double> Comm::getSum(const Vector2<double> &val) {
#ifdef PARTICLE_SIMULATOR_MPI_PARALLEL
  Vector2<double> ret;
...
/*
    template<> inline void Comm::getMinValue<float, int>(const float & f_in,
   const int & i_in, float & f_out, int & i_out){ allreduceMin(f_in, i_in,
   f_out, i_out);
    }
    template<> inline void Comm::getMinValue<double, int>(const double & f_in,
   const int & i_in, double & f_out, int & i_out){ allreduceMin(f_in, i_in,
   f_out, i_out);
    }
*/
template <>
inline void Comm::getMinValue<float>(const float &f_in, const int &i_in,
                                     float &f_out, int &i_out) {
  allreduceMin(f_in, i_in, f_out, i_out);
}
template <>
inline void Comm::getMinValue<double>(const double &f_in, const int &i_in,
...
*/
template <>
inline void Comm::getMaxValue<float>(const float &f_in, const int &i_in,
                                     float &f_out, int &i_out) {
  allreduceMax(f_in, i_in, f_out, i_out);
}

template <>
inline void Comm::getMaxValue<double>(const double &f_in, const int &i_in,
                                      double &f_out, int &i_out) {
  allreduceMax(f_in, i_in, f_out, i_out);
}
...
class TimeProfile {
 public:
  ...
  void dump(std::ostream &fout = std::cout) const {
    ...
    fout << "set_particle_global_tree= " << set_particle_global_tree
         << std::endl;
  ...
  TimeProfile() {
    collect_sample_particle = decompose_domain = exchange_particle =
        set_particle_local_tree = set_particle_global_tree = make_local_tree =
            make_global_tree = set_root_cell = calc_force =
                calc_moment_local_tree = calc_moment_global_tree =
                    make_LET_1st = make_LET_2nd = exchange_LET_1st =
                        exchange_LET_2nd = 0.0;
    ...
    exchange_particle__find_particle = exchange_particle__exchange_particle =
        0.0;

    decompose_domain__sort_particle_1st = decompose_domain__sort_particle_2nd =
        decompose_domain__sort_particle_3rd =
            decompose_domain__gather_particle = 0.0;
  ...
  TimeProfile operator+(const TimeProfile &rhs) const {
    ...
    ret.make_local_tree_tot =
        this->make_local_tree_tot + rhs.make_local_tree_tot;
    ...
    ret.calc_force__copy_original_order =
        this->calc_force__copy_original_order +
  ...
  F64 getTotalTime() const {
    ...
    make_global_tree + set_root_cell
        + calc_force + calc_moment_local_tree + calc_moment_global_tree +
    make_LET_1st + make_LET_2nd + exchange_LET_1st + exchange_LET_2nd;
    */
    return collect_sample_particle + decompose_domain + exchange_particle +
           set_particle_local_tree + set_particle_global_tree +
           make_local_tree + make_global_tree + set_root_cell + calc_force +
