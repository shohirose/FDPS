 ...
 public:
  ...
  /*
          ...
          void exchangeLet(const bool reuse = false){
              ...
              Comm::allToAllV(ep_send_.getPointer(), n_ep_send_,
     n_disp_ep_send_, ep_recv_.getPointer(), n_ep_recv_, n_disp_ep_recv_);
              Comm::allToAllV(sp_send_.getPointer(), n_sp_send_,
     n_disp_sp_send_, sp_recv_.getPointer(), n_sp_recv_, n_disp_sp_recv_);
          }
          ...
          void allgahterTreeOuterPos(const F64ort & _tree_outer_pos){
              Comm::allGather(&_tree_outer_pos, 1,
     tree_outer_pos_.getPointer());
          }
          ...
          void dumpMemSizeUsed(std::ostream & fout){
              ...
              fout<<"rank_sp_isend_.getMemSize()=
     "<<rank_sp_isend_.getMemSize()<<std::endl;
              fout<<"rank_sp_irecv_.getMemSize()=
     "<<rank_sp_irecv_.getMemSize()<<std::endl;
              fout<<"rank_sp_top_recv_.getMemSize()=
     "<<rank_sp_top_recv_.getMemSize()<<std::endl;
              fout<<"rank_ep_send_.getMemSize()=
     "<<rank_ep_send_.getMemSize()<<std::endl;
              fout<<"rank_ep_recv_.getMemSize()=
     "<<rank_ep_recv_.getMemSize()<<std::endl; fout<<"sum= "<<
