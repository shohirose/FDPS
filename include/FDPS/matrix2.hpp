...
class Matrix2 {
 public:
  ...
  Matrix2(const Matrix2& src)
      : xx(src.xx), yy(src.yy), xy(src.xy), yx(src.yx) {}

  Matrix2(const Vector2<T>& a, const Vector2<T>& b)
      : xx(a.x * b.x), yy(a.y * b.y), xy(a.x * b.y), yx(a.y * b.x) {}
  ...
  Matrix2 operator*(const Matrix2& rhs) const {
    return Matrix2(xx * rhs.xx + xy * rhs.yx, yx * rhs.xy + yy * rhs.yy,
                   xx + rhs.xy + xy * rhs.yy, yx * rhs.xx + yy * rhs.yx);
  }
  ...
  friend Matrix2 operator*(const T s, const Matrix2& m) { return (m * s); }
  ...
  const Matrix2& operator+() const { return (*this); }

  const Matrix2 operator-() const { return Matrix2(-xx, -yy, -xy, -yx); }

  T getTrace() const { return (xx + yy); }

  T getDeterminant() const { return (xx * yy - xy * yx); }

  T getSecondInvariant() const {
    return 0.5 *
           (-this->getTrace() * this->getTrace() + (*this * *this).getTrace());
  }

  template <typename U>
  operator Matrix2<U>() const {
    return Matrix2<U>(static_cast<U>(xx), static_cast<U>(yy),
                      static_cast<U>(xy), static_cast<U>(yx));
  }
