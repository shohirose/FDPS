...
class TreeParticle {
 public:
  U64 key_;
  U32 adr_ptcl_;  // U32 because its MSB is used for distinguish if it is EP or
                  // SP
  ...
  U64 getKey() const { return key_; }

  // for DEBUG
  void setKey(const U64 key) { key_ = key; }
...
class TreeCell {
 public:
  ...
  void dumpTree(const Tep* const first_ep_ptr,
                const TreeCell* const first_tc_ptr, const F32vec& center,
                const F32 half_length, const S32 n_leaf_limit,
    ...
    if (!(this->isLeaf(n_leaf_limit))) {
      ...
      for (S32 ic = 0; ic < N_CHILDREN; ic++) {
        ...
        (child + ic)
            ->dumpTree(first_ep_ptr, first_tc_ptr,
                       center + SHIFT_CENTER[ic] * half_length,
                       half_length * 0.5, n_leaf_limit, fout);
      }
    } else {
  ...
  void checkTree(const Tep* const first_ep_ptr,
                 const TreeCell* const first_tc_ptr, const F32vec& center,
                 const F32 half_length, const S32 n_leaf_limit,
                 const F32 tolerance, S32& err,
                 std::ostream& fout = std::cout) const {
    if (!(this->isLeaf(n_leaf_limit))) {
      // std::cerr<<"adr_tc_="<<adr_tc_<<std::endl;
      const TreeCell* child = first_tc_ptr + adr_tc_;
      for (S32 ic = 0; ic < N_CHILDREN; ic++) {
        // std::cerr<<"(child + ic)->n_ptcl_="<<(child +
        // ic)->n_ptcl_<<std::endl;
        if ((child + ic)->n_ptcl_ <= 0) continue;
        const Tep* ptcl = first_ep_ptr + adr_ptcl_;
        for (S32 ip = 0; ip < n_ptcl_; ip++, ptcl++) {
          if (!IsInBox(ptcl->getPos(), center, half_length, tolerance)) {
            ...
            fout << "center=" << center << std::endl;
            fout << "half_length=" << half_length << std::endl;
            fout << "(center+half_length)-ptcl->getPos()="
                 << (center + half_length) - ptcl->getPos() << std::endl;
            fout << "(center-half_length)-ptcl->getPos()="
                 << (center - half_length) - ptcl->getPos() << std::endl;
            err++;
          }
        ...
        (child + ic)
            ->checkTree(first_ep_ptr, first_tc_ptr,
                        center + SHIFT_CENTER[ic] * half_length,
  ...
  void checkTreeLongGlobalTree(const Tep* const first_ep_ptr,
                               ...
                               const F32vec& center, const F32 half_length,
                               const S32 n_leaf_limit, const F32 tolerance,
                               S32& err, std::ostream& fout = std::cout) const {
    if (!(this->isLeaf(n_leaf_limit))) {
      const TreeCell* child = first_tc_ptr + adr_tc_;
      for (S32 ic = 0; ic < N_CHILDREN; ic++) {
        if ((child + ic)->n_ptcl_ <= 0) continue;
        ...
        for (S32 ip = 0; ip < n_ptcl_; ip++, tp_tmp++) {
          ...
          */
          ...
          if (!IsInBox(pos_tmp, center, half_length, tolerance)) {
            ...
            fout << "(center+half_length)-pos_tmp="
                 << (center + half_length) - pos_tmp << std::endl;
            fout << "(center-half_length)-pos_tmp="
                 << (center - half_length) - pos_tmp << std::endl;
            err++;
            //}
...
class MomentDipoleGeometricCenter {
 public:
  ...
  MomentDipoleGeometricCenter(const FSP c, const FSPvec& p, const SSP n,
                              const FSPvec& di) {
    charge = c;
    pos = p;
    n_ptcl = n;
...
class MomentQuadrupoleGeometricCenter {
 public:
  ...
  MomentQuadrupoleGeometricCenter(const FSP c, const FSPvec& p, const SSP n,
                                  const FSPvec& di, const FSPmat& q) {
    charge = c;
    pos = p;
    n_ptcl = n;
    dipole = di;
...
class MomentMonopoleCutoff {
 public:
  ...
  MomentMonopoleCutoff(const FSP m, const FSPvec& p, const F64ort& v_out) {
    mass = m;
    pos = p;
    vertex_out_ = v_out;
  }
...
class SPJQuadrupoleGeometricCenter {
 public:
  ...
  MomentQuadrupoleGeometricCenter convertToMoment() const {
    return MomentQuadrupoleGeometricCenter(charge, pos, n_ptcl, dipole,
                                           quadrupole);
  }
};
