...
class Orthotope2 {
 public:
  ...
  Orthotope2(const Vector2<T>& center, const T length)
      : low_(center - (Vector2<T>)(length)),
        high_(center + (Vector2<T>)(length)) {}
  ...
  */
  ...
  const Vector2<T> getCenter() const { return (high_ + low_) * T(0.5); }

  const Vector2<T> getHalfLength() const { return (high_ - low_) * T(0.5); }

  const Vector2<T> getFullLength() const { return (high_ - low_); }
  ...
  const T getDistanceMinSQ(const Vector2<T>& vec) const {
    T dx = (vec.x > high_.x) ? (vec.x - high_.x)
                             : ((vec.x < low_.x) ? (low_.x - vec.x) : T(0));
    T dy = (vec.y > high_.y) ? (vec.y - high_.y)
                             : ((vec.y < low_.y) ? (low_.y - vec.y) : T(0));
    return dx * dx + dy * dy;
  }
