...
class RadixSort {
 ...
 public:
  ...
  template <class Tobj>
  void lsdSort(Tobj* val, Tobj* val_buf, const int first, const int last) {
...
#pragma omp single
#endif
        {
          ...
          for (int i = 1; i < n_bucket_; i++) {
            prefix_sum_[0][i] = prefix_sum_[n_thread_ - 1][i - 1] +
                                bucket_size_[n_thread_ - 1][i - 1];
            for (int j = 1; j < n_thread_; j++) {
              prefix_sum_[j][i] =
                  prefix_sum_[j - 1][i] + bucket_size_[j - 1][i];
            }
          }
        }
#ifdef PARTICLE_SIMULATOR_THREAD_PARALLEL
