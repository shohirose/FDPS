...
class Orthotope3 {
 public:
  ...
  Orthotope3(const Vector3<T>& center, const T length)
      : low_(center - (Vector3<T>)(length)),
        high_(center + (Vector3<T>)(length)) {}
  ...
  void merge(const Vector3<T>& vec, const T size) {
    this->high_.x =
        (this->high_.x > vec.x + size) ? this->high_.x : vec.x + size;
    this->high_.y =
        (this->high_.y > vec.y + size) ? this->high_.y : vec.y + size;
    this->high_.z =
        (this->high_.z > vec.z + size) ? this->high_.z : vec.z + size;
  ...
  unsigned int notContained(const Vector3<T>& pos) const {
    return (pos.x < low_.x) || (high_.x <= pos.x) || (pos.y < low_.y) ||
           (high_.y <= pos.y) || (pos.z < low_.z) || (high_.z <= pos.z);
  }
  ...
  const Vector3<T> getCenter() const { return (high_ + low_) * T(0.5); }

  const Vector3<T> getHalfLength() const { return (high_ - low_) * T(0.5); }

  const Vector3<T> getFullLength() const { return (high_ - low_); }
  ...
  const T getDistanceMinSQ(const Vector3<T>& vec) const {
    const Vector3<T> b_high = this->high_;
    const Vector3<T> b_low = this->low_;
    T dx = (vec.x > b_high.x) ? (vec.x - b_high.x)
                              : ((vec.x < b_low.x) ? (b_low.x - vec.x) : T(0));
    T dy = (vec.y > b_high.y) ? (vec.y - b_high.y)
                              : ((vec.y < b_low.y) ? (b_low.y - vec.y) : T(0));
    T dz = (vec.z > b_high.z) ? (vec.z - b_high.z)
                              : ((vec.z < b_low.z) ? (b_low.z - vec.z) : T(0));
    return dx * dx + dy * dy + dz * dz;
  }
