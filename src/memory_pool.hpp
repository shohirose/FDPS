...
class MemoryPool {
 private:
  ...
  static bool isLastSegment(const int id_seg) {
    if (id_seg < N_SEGMENT_LIMIT && getInstance().n_segment_ > 0 &&
 ...
 public:
  static size_t getNSegment() { return getInstance().n_segment_; }

  static size_t getSize() { return getInstance().size_; }
  ...
  static void freeMem(const int id_seg) {
    ...
    if (id_seg < N_SEGMENT_LIMIT) {
      ...
      getInstance().top_ =
          ((char *)getInstance().bottom_) + getInstance().size_;
    } else {
