...
size_t MemoryPool::getAlignSize(const size_t _size) noexcept {
  return (((_size - 1) / ALIGN_SIZE) + 1) * ALIGN_SIZE;
}

bool MemoryPool::isLastSegment(const int id_seg) noexcept {
  return (id_seg < N_SEGMENT_LIMIT && getInstance().n_segment_ > 0 &&
          (size_t)id_seg == getInstance().n_segment_ - 1);
}

bool MemoryPool::inParallelRegion() noexcept {
...
void MemoryPool::initialize(const size_t _cap) noexcept {
...
void MemoryPool::reInitialize(const size_t size_add) noexcept {
...
void MemoryPool::unifyMem() noexcept {
  const size_t n_emerg_bufs = getInstance().emerg_bufs_.size();
  if (!inParallelRegion() && n_emerg_bufs > 0) {
    size_t size_add = 0;
    for (size_t i = 0; i < n_emerg_bufs; i++) {
    ...
    for (size_t i = 0; i < n_emerg_bufs; i++) {
      ...
      const size_t id_mpool = getInstance().n_segment_;
...
void MemoryPool::alloc(const size_t _size, int &_id_mpool, void *ptr_data,
                       void *&ret) noexcept {
  if (_size <= 0) return;
  unifyMem();
  ...
  if (!flag_break) {
    ...
    if (isLastSegment(_id_mpool)) {
      getInstance().n_segment_--;
      getInstance().top_ = ((char *)getInstance().top_) -
                           getInstance().cap_per_seg_[getInstance().n_segment_];
      getInstance().size_ =
          getInstance().size_ -
          getInstance().cap_per_seg_[getInstance().n_segment_];
    ...
    if (!flag_use_emerg_bufs) {
      ...
      getInstance().used_per_seg_[getInstance().n_segment_] = true;
      getInstance().ptr_data_per_seg_[getInstance().n_segment_] = ptr_data;
      _id_mpool = static_cast<int>(getInstance().n_segment_);
      getInstance().n_segment_++;
      ret = top_prev;
    } else {
      size_t n_emerg_bufs = getInstance().emerg_bufs_.size();
      // Newly create an emergency buffer
      const size_t idx = n_emerg_bufs;
      n_emerg_bufs++;
      ...
      if (data != NULL) {
        ...
        _id_mpool = static_cast<int>(idx) + N_SEGMENT_LIMIT;
...
void MemoryPool::freeMem(const int id_seg) noexcept {
...
void MemoryPool::dump() noexcept {
  ...
  for (size_t i = 0; i < getInstance().n_segment_; i++) {
    std::cerr << "i= " << i << " cap= " << getInstance().cap_per_seg_[i]
              << " used= " << getInstance().used_per_seg_[i] << std::endl;
  }
}
