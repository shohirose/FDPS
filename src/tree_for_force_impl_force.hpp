...
void CalcForceMultiWalkInitialize(ReallocatableArray<Tforce>& force_sorted,
                                  const S32 n_loc_tot, const bool tag_max,
                                  const bool clear) {
  if (tag_max <= 0) {
    PARTICLE_SIMULATOR_PRINT_ERROR(
        "tag_max is illegal. In currente version, tag_max must be 1");
    Abort(-1);
  }
...
S32 TreeForForce<TSM, Tforce, Tepi, Tepj, Tmomloc, Tmomglb,
                 Tspj>::calcForceMultiWalkIndex(Tfunc_dispatch pfunc_dispatch,
  ...
  ret = calcForceMultiWalkIndexImpl(typename TSM::force_type(), pfunc_dispatch,
                                    pfunc_retrieve, tag_max, n_walk_limit,
                                    flag_keep_list, clear);
...
S32 TreeForForce<TSM, Tforce, Tepi, Tepj, Tmomloc, Tmomglb, Tspj>::
    calcForceMultiWalkIndexImpl(TagForceLong, Tfunc_dispatch pfunc_dispatch,
                                Tfunc_retrieve pfunc_retrieve,
                                const S32 tag_max, const S32 n_walk_limit,
                                const bool flag_keep_list, const bool clear) {
  ...
  pfunc_dispatch(0, 0, (const Tepi**)epi_dummy, n_epi_dummy,
                 ...
                 spj_sorted_.getPointer(), spj_sorted_.size(), true);
  S32 ret = 0;
  S32 tag = 0;
  ...
  const S32 n_loop_max =
      n_ipg / n_walk_limit + ((n_ipg % n_walk_limit) == 0 ? 0 : 1);
  ...
  ptr_force_per_walk[0].initialize(n_walk_limit, n_walk_limit,
                                   1);  // array of pointer *[n_walk]
  ptr_force_per_walk[1].initialize(n_walk_limit, n_walk_limit,
                                   1);  // array of pointer *[n_walk]
  ...
  if (n_ipg > 0) {
    for (int wg = 0; wg < n_loop_max; wg++) {
      const S32 n_walk =
          n_ipg / n_loop_max + (((n_ipg % n_loop_max) > wg) ? 1 : 0);
      const S32 walk_grp_head =
          (n_ipg / n_loop_max) * wg + std::min((n_ipg % n_loop_max), wg);
...
#pragma omp for schedule(dynamic, 4) reduction(+: n_interaction_ep_ep_tmp) reduction(+: n_interaction_ep_sp_tmp)
        ...
        for (int iw = 0; iw < n_walk; iw++) {
          ...
          ptr_force_per_walk[lane_now][iw] =
          ...
          MakeListUsingTreeRecursiveTop<
              ...
              tc_glb_, adr_tc, tp_glb_, epj_sorted_, adr_epj_for_force_[ith],
              spj_sorted_, adr_spj_for_force_[ith], target_box, r_crit_sq,
              n_leaf_limit_, adr_tree_sp_first, F64vec(0.0));
          ...
          n_interaction_ep_ep_tmp +=
              ((S64)n_epj_per_walk[iw] * (S64)n_epi_per_walk[lane_now][iw]);
          n_interaction_ep_sp_tmp +=
              ((S64)n_spj_per_walk[iw] * (S64)n_epi_per_walk[lane_now][iw]);
          iwloc2iw[n_walk_loc] = iw;
          n_walk_loc++;
        }  // end of OMP for
        for (S32 iwloc = 0; iwloc < n_walk_loc; iwloc++) {
          S32 iw = iwloc2iw[iwloc];
          ptr_adr_epj_per_walk[iw] = adr_epj_for_force_[ith].getPointer(
              n_epj_disp_per_thread[ith][iwloc]);
          ptr_adr_spj_per_walk[iw] = adr_spj_for_force_[ith].getPointer(
              n_spj_disp_per_thread[ith][iwloc]);
        }
...
#pragma omp parallel for
        ...
        for (int iw = 0; iw < n_walk; iw++) {
          const S32 id_ipg = walk_grp_head + iw;
          interaction_list_.n_disp_ep_[id_ipg + 1] =
              interaction_list_.n_disp_ep_[id_ipg] +
              interaction_list_.n_ep_[id_ipg];
          interaction_list_.n_disp_sp_[id_ipg + 1] =
              interaction_list_.n_disp_sp_[id_ipg] +
              interaction_list_.n_sp_[id_ipg];
        }
        interaction_list_.adr_ep_.resizeNoInitialize(
            interaction_list_.n_disp_ep_[walk_grp_head + n_walk]);
        interaction_list_.adr_sp_.resizeNoInitialize(
            interaction_list_.n_disp_sp_[walk_grp_head + n_walk]);
#ifdef PARTICLE_SIMULATOR_THREAD_PARALLEL
#pragma omp parallel for
#endif
      ...
      time_profile_.calc_force__core__walk_tree +=
          GetWtime() - offset_calc_force__core__walk_tree;
      if (!first_loop) {
        ret += pfunc_retrieve(tag, n_walk_prev,
                              n_epi_per_walk[lane_old].getPointer(),
                              ptr_force_per_walk[lane_old].getPointer());
      }  // retrieve
      pfunc_dispatch(0, n_walk, (const Tepi**)ptr_epi_per_walk.getPointer(),
                     n_epi_per_walk[lane_now].getPointer(),
                     (const S32**)ptr_adr_epj_per_walk.getPointer(),
                     n_epj_per_walk.getPointer(),
                     (const S32**)ptr_adr_spj_per_walk.getPointer(),
                     n_spj_per_walk.getPointer(), epj_sorted_.getPointer(),
                     epj_sorted_.size(), spj_sorted_.getPointer(),
                     spj_sorted_.size(), false);
      first_loop = false;
      n_walk_prev = n_walk;
    }  // end of walk group loop
    ret += pfunc_retrieve(
        tag, n_walk_prev, n_epi_per_walk[(n_loop_max + 1) % 2].getPointer(),
  ...
  time_profile_.calc_force__copy_original_order +=
      GetWtime() - offset_copy_original_order;
...
template <class Tfunc_dispatch, class Tfunc_retrieve>
S32 TreeForForce<TSM, Tforce, Tepi, Tepj, Tmomloc, Tmomglb, Tspj>::
    calcForceMultiWalkIndexImpl(TagForceShort, Tfunc_dispatch pfunc_dispatch,
                                Tfunc_retrieve pfunc_retrieve,
                                const S32 tag_max, const S32 n_walk_limit,
                                const bool flag_keep_list, const bool clear) {
  const F64 offset_core = GetWtime();
  ...
  pfunc_dispatch(0, 0, (const Tepi**)epi_dummy, n_epi_dummy,
                 (const S32**)id_epj_dummy, n_epj_dummy,
                 epj_sorted_.getPointer(), epj_sorted_.size(), true);
...
#pragma omp for schedule(dynamic, 4) reduction(+ : n_interaction_ep_ep_tmp)
        ...
        for (int iw = 0; iw < n_walk; iw++) {
          ...
          target_box.set(ipg_[id_ipg]);
          S32 adr_tc = 0;
          MakeListUsingTreeRecursiveTop<TSM, TreeCell<Tmomglb>, TreeParticle,
                                        Tepj, WALK_MODE_NORMAL,
                                        TagChopLeafTrue>(
              tc_glb_, adr_tc, tp_glb_, epj_sorted_, adr_epj_for_force_[ith],
              target_box, r_crit_sq, n_leaf_limit_, F64vec(0.0));
          n_epj_per_walk[iw] = adr_epj_for_force_[ith].size() - n_ep_cum;
          n_ep_cum = adr_epj_for_force_[ith].size();
          n_epj_disp_per_thread[ith][n_walk_loc + 1] = n_ep_cum;
          n_interaction_ep_ep_tmp +=
        ...
        for (S32 iwloc = 0; iwloc < n_walk_loc; iwloc++) {
          S32 iw = iwloc2iw[iwloc];
          adr_epj_per_walk[iw] = adr_epj_for_force_[ith].getPointer(
              n_epj_disp_per_thread[ith][iwloc]);
        }
      }  // end of OMP parallel scope
      if (flag_keep_list) {
...
#pragma omp parallel for
      ...
      pfunc_dispatch(0, n_walk, (const Tepi**)epi_per_walk.getPointer(),
                     n_epi_per_walk[lane_now].getPointer(),
                     (const S32**)adr_epj_per_walk.getPointer(),
                     n_epj_per_walk.getPointer(), epj_sorted_.getPointer(),
...
template <class Tfunc_dispatch, class Tfunc_retrieve>
S32 TreeForForce<TSM, Tforce, Tepi, Tepj, Tmomloc, Tmomglb,
                 Tspj>::calcForceMultiWalkPtcl(Tfunc_dispatch pfunc_dispatch,
                                               Tfunc_retrieve pfunc_retrieve,
                                               const S32 tag_max,
                                               const S32 n_walk_limit,
                                               const bool flag_keep_list,
  ...
  ret = calcForceMultiWalkPtclImpl(typename TSM::force_type(), pfunc_dispatch,
                                   pfunc_retrieve, tag_max, n_walk_limit,
                                   flag_keep_list, clear);
...
template <class Tfunc_dispatch, class Tfunc_retrieve>
S32 TreeForForce<TSM, Tforce, Tepi, Tepj, Tmomloc, Tmomglb, Tspj>::
    calcForceMultiWalkPtclImpl(TagForceLong, Tfunc_dispatch pfunc_dispatch,
...
#pragma omp for schedule(dynamic, 4) reduction(+: n_interaction_ep_ep_tmp) reduction(+: n_interaction_ep_sp_tmp)
        ...
        for (S32 iw_loc = 0; iw_loc < n_walk_loc; iw_loc++) {
          ...
          ptr_epj_per_walk[iw] = epj_for_force_[ith].getPointer(
              n_epj_disp_per_thread[ith][iw_loc]);
          ptr_spj_per_walk[iw] = spj_for_force_[ith].getPointer(
              n_spj_disp_per_thread[ith][iw_loc]);
        }
      }  // end of OMP parallel scope
      if (flag_keep_list) {
        interaction_list_.n_disp_ep_[0] = interaction_list_.n_disp_sp_[0] = 0;
...
#pragma omp parallel for
      ...
      //               (const Tepi**)ptr_epi_per_walk.getPointer(),
      //               n_epi_per_walk[lane_now].getPointer(), (const
      //               S32**)ptr_adr_epj_per_walk.getPointer(),
      //               n_epj_per_walk.getPointer(), (const
      //               S32**)ptr_adr_spj_per_walk.getPointer(),
      //               n_spj_per_walk.getPointer(), epj_sorted_.getPointer(),
      //               epj_sorted_.size(), spj_sorted_.getPointer(),
      //               spj_sorted_.size(), false);
      ret += pfunc_dispatch(tag, n_walk,
                            ...
                            n_epj_per_walk.getPointer(),
                            (const Tspj**)ptr_spj_per_walk.getPointer(),
                            n_spj_per_walk.getPointer());
      first_loop = false;
      n_walk_prev = n_walk;
    }  // end of walk group loop
    ret += pfunc_retrieve(
...
template <class Tfunc_dispatch, class Tfunc_retrieve>
S32 TreeForForce<TSM, Tforce, Tepi, Tepj, Tmomloc, Tmomglb, Tspj>::
    calcForceMultiWalkPtclImpl(TagForceShort, Tfunc_dispatch pfunc_dispatch,
                               Tfunc_retrieve pfunc_retrieve, const S32 tag_max,
                               const S32 n_walk_limit,
                               const bool flag_keep_list, const bool clear) {
  const F64 offset_core = GetWtime();
  ...
  ReallocatableArray<Tepi*> ptr_epi_per_walk(n_walk_limit, n_walk_limit, 1);
  ReallocatableArray<S32> n_epj_per_walk(n_walk_limit, n_walk_limit, 1);
  ReallocatableArray<Tepj*> ptr_epj_per_walk(n_walk_limit, n_walk_limit,
                                             1);  // new
  ReallocatableArray<S32>* n_epj_disp_per_thread;
  ptr_force_per_walk[0].initialize(n_walk_limit, n_walk_limit,
                                   1);  // array of pointer *[n_walk]
  ...
  if (n_ipg > 0) {
    ...
    for (int wg = 0; wg < n_loop_max; wg++) {
      ...
      for (S32 iw = 0; iw < n_walk; iw++) {
        ...
        id_epj_ar[iw] = interaction_list_.adr_ep_.getPointer(
            interaction_list_.n_disp_ep_[id_walk]);

        n_spj_ar[iw] = interaction_list_.n_sp_[id_walk];
        id_spj_ar[iw] = interaction_list_.adr_sp_.getPointer(
            interaction_list_.n_disp_sp_[id_walk]);

        n_interaction_ep_ep_local_ +=
            ((S64)n_epj_ar[iw] * (S64)n_epi_ar[lane_0][iw]);
        n_interaction_ep_sp_local_ +=
            ((S64)n_spj_ar[iw] * (S64)n_epi_ar[lane_0][iw]);
      }
      if (!first_loop) {
        ret += pfunc_retrieve(tag, n_walk_prev, n_epi_ar[lane_1].getPointer(),
                              force_ar[lane_1].getPointer());
      }
      pfunc_dispatch(0, n_walk, (const Tepi**)epi_ar.getPointer(),
                     n_epi_ar[lane_0].getPointer(),
                     (const S32**)id_epj_ar.getPointer(), n_epj_ar.getPointer(),
                     (const S32**)id_spj_ar.getPointer(), n_spj_ar.getPointer(),
                     epj_sorted_.getPointer(), epj_sorted_.size(),
                     spj_sorted_.getPointer(), spj_sorted_.size(), false);
      n_walk_prev = n_walk;
      first_loop = false;
    }
    ret += pfunc_retrieve(tag, n_walk, n_epi_ar[lane_0].getPointer(),
                          force_ar[lane_0].getPointer());
  }
  ...
  if (n_ipg > 0) {
    ...
    for (int wg = 0; wg < n_loop_max; wg++) {
      ...
      pfunc_dispatch(0, n_walk, (const Tepi**)epi_ar.getPointer(),
                     n_epi_ar[lane_0].getPointer(),
                     (const S32**)id_epj_ar.getPointer(), n_epj_ar.getPointer(),
                     epj_sorted_.getPointer(), epj_sorted_.size(), false);
      n_walk_prev = n_walk;
      first_loop = false;
    }
    ret += pfunc_retrieve(tag, n_walk, n_epi_ar[lane_0].getPointer(),
  ...
  if (n_ipg > 0) {
    ...
    for (int wg = 0; wg < n_loop_max; wg++) {
      ...
      for (S32 jp = 0; jp < n_epj_tot; jp++) {
        epj_for_force_[0][jp] =
            epj_sorted_[interaction_list_.adr_ep_[jp + n_ep_head]];
      }
      ...
      for (S32 jp = 0; jp < n_spj_tot; jp++) {
        spj_for_force_[0][jp] =
            spj_sorted_[interaction_list_.adr_sp_[jp + n_sp_head]];
      }
      S64 n_epj_cnt = 0;
      S64 n_spj_cnt = 0;
      ...
      ret += pfunc_dispatch(
          tag, n_walk, (const Tepi**)epi_ar.getPointer(),
          n_epi_ar[lane_0].getPointer(), (const Tepj**)epj_ar.getPointer(),
          n_epj_ar.getPointer(), (const Tspj**)spj_ar.getPointer(),
          n_spj_ar.getPointer());
      first_loop = false;
    }
    ret += pfunc_retrieve(tag, n_walk, n_epi_ar[lane_0].getPointer(),
...
#pragma omp parallel for
  ...
  if (n_ipg > 0) {
    ...
    for (int wg = 0; wg < n_loop_max; wg++) {
      ...
      ret += pfunc_dispatch(tag, n_walk, (const Tepi**)epi_ar.getPointer(),
                            n_epi_ar[lane_0].getPointer(),
                            (const Tepj**)epj_ar.getPointer(),
                            n_epj_ar.getPointer());
      first_loop = false;
    }
    ret += pfunc_retrieve(tag, n_walk, n_epi_ar[lane_0].getPointer(),
...
void TreeForForce<TSM, Tforce, Tepi, Tepj, Tmomloc, Tmomglb, Tspj>::calcForce(
    Tfunc_ep_ep pfunc_ep_ep, const bool clear) {
  F64 time_offset = GetWtime();
  force_sorted_.resizeNoInitialize(n_loc_tot_);
  const S64 n_ipg = ipg_.size();
  n_walk_local_ += n_ipg;
...
#pragma omp parallel for schedule(dynamic, 4) reduction(+ : ni_tmp, nj_tmp, n_interaction_ep_ep_tmp)
    ...
    for (S32 i = 0; i < n_ipg; i++) {
      ...
      time_profile_.calc_force__core__walk_tree +=
          GetWtime() - offset_walk_tree;
      ni_tmp += ipg_[i].n_ptcl_;
      nj_tmp += epj_for_force_[Comm::getThreadNum()].size();
      n_interaction_ep_ep_tmp +=
          ipg_[i].n_ptcl_ * epj_for_force_[Comm::getThreadNum()].size();
    ...
    n_interaction_ep_ep_local_ += n_interaction_ep_ep_tmp;
    for (S32 i = 1; i < Comm::getNumberOfThread(); i++)
      n_cell_open_[0] += n_cell_open_[i];
  } else {
    ni_ave_ = nj_ave_ = n_interaction_ep_ep_ = 0;
...
void TreeForForce<TSM, Tforce, Tepi, Tepj, Tmomloc, Tmomglb,
                  Tspj>::calcForceOnly(Tfunc_ep_ep pfunc_ep_ep,
                                       const S32 adr_ipg, const bool clear) {
...
template <class TSM, class Tforce, class Tepi, class Tepj, class Tmomloc,
          class Tmomglb, class Tspj>
void TreeForForce<TSM, Tforce, Tepi, Tepj, Tmomloc, Tmomglb,
                  Tspj>::makeInteractionListImpl(TagForceShort,
                                                 const S32 adr_ipg,
                                                 const bool clear) {
  const S32 ith = Comm::getThreadNum();
  if (clear) {
  ...
  CopyPjForForceST(adr_epj_for_force_[ith], epj_sorted_, n_head, n_tail,
                   epj_for_force_[ith]);
}
...
void TreeForForce<TSM, Tforce, Tepi, Tepj, Tmomloc, Tmomglb, Tspj>::calcForce(
    Tfunc_ep_ep pfunc_ep_ep, Tfunc_ep_sp pfunc_ep_sp, const bool clear) {
  const F64 time_offset = GetWtime();
  force_sorted_.resizeNoInitialize(n_loc_tot_);
  const S64 n_ipg = ipg_.size();
  n_walk_local_ += n_ipg;
...
#pragma omp parallel for schedule(dynamic, 4) reduction(+ : ni_tmp, nj_tmp, n_interaction_ep_ep_tmp, n_interaction_ep_sp_tmp)
    ...
    for (S32 i = 0; i < n_ipg; i++) {
      ...
      n_interaction_ep_ep_tmp +=
          ipg_[i].n_ptcl_ * epj_for_force_[Comm::getThreadNum()].size();
      n_interaction_ep_sp_tmp +=
          ipg_[i].n_ptcl_ * spj_for_force_[Comm::getThreadNum()].size();
...
template <class TSM, class Tforce, class Tepi, class Tepj, class Tmomloc,
          class Tmomglb, class Tspj>
void TreeForForce<TSM, Tforce, Tepi, Tepj, Tmomloc, Tmomglb,
                  Tspj>::makeInteractionListImpl(TagForceLong,
                                                 const S32 adr_ipg,
                                                 const bool clear) {
  const S32 ith = Comm::getThreadNum();
  if (clear) {
  ...
  if (theta_ > 0.0) {
    ...
    CopyPjForForceST(adr_epj_for_force_[ith], epj_sorted_, n_epj_head,
                     n_epj_tail, epj_for_force_[ith]);
    CopyPjForForceST(adr_spj_for_force_[ith], spj_sorted_, n_spj_head,
                     n_spj_tail, spj_for_force_[ith]);
  } else {
    // theta_ = 0 case
    makeInteractionListLongForZeroTheta(
        typename TraitsForCutoff<typename TSM::search_type>::type_cutoff(),
        adr_ipg);
  }
}
...
template <class Tfunc_ep_ep>
void TreeForForce<TSM, Tforce, Tepi, Tepj, Tmomloc, Tmomglb,
                  Tspj>::calcForceNoWalk(Tfunc_ep_ep pfunc_ep_ep,
                                         const bool clear) {
  F64 time_offset = GetWtime();
  force_sorted_.resizeNoInitialize(n_loc_tot_);
  // force_org_.resizeNoInitialize(n_loc_tot_);
