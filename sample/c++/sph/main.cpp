...
struct FP {
  ...
  PS::F64vec vel_half;
  PS::F64 eng_half;
  void copyFromForce(const Dens& tdens) { this->dens = tdens.dens; }
  ...
  PS::F64 getRSearch() const { return kernelSupportRadius * this->smth; }
  void setPos(const PS::F64vec& tpos) { this->pos = tpos; }
  void writeAscii(FILE* fp) const {
    fprintf(fp,
            "%lld\t%lf\t%lf\t%lf\t%lf\t%lf\t"
            "%lf\t%lf\t%lf\t%lf\t%lf\n",
            this->id, this->mass, this->pos.x, this->pos.y, this->pos.z,
            this->vel.x, this->vel.y, this->vel.z, this->dens, this->eng,
            this->pres);
  }
...
class CalcDensity {
 public:
  void operator()(const EP* const ep_i, const PS::S32 Nip, const EP* const ep_j,
                  const PS::S32 Njp, Dens* const dens) {
...
class CalcHydroForce {
 public:
  void operator()(const EP* const ep_i, const PS::S32 Nip, const EP* const ep_j,
                  const PS::S32 Njp, Hydro* const hydro) {
    for (PS::S32 i = 0; i < Nip; ++i) {
      hydro[i].clear();
      ...
      for (PS::S32 j = 0; j < Njp; ++j) {
        ...
        const PS::F64 AV =
            -0.5 * v_sig * w_ij / (0.5 * (ep_i[i].dens + ep_j[j].dens));
        const PS::F64vec gradW_ij =
            0.5 * (gradW(dr, ep_i[i].smth) + gradW(dr, ep_j[j].smth));
        hydro[i].acc -= ep_j[j].mass *
                        (ep_i[i].pres / (ep_i[i].dens * ep_i[i].dens) +
                         ep_j[j].pres / (ep_j[j].dens * ep_j[j].dens) + AV) *
                        gradW_ij;
        hydro[i].eng_dot +=
            ep_j[j].mass *
            (ep_i[i].pres / (ep_i[i].dens * ep_i[i].dens) + 0.5 * AV) * dv *
            gradW_ij;
      }
...
void SetupIC(PS::ParticleSystem<FP>& sph_system, PS::F64* end_time,
             boundary* box) {
  // Place SPH particles
  std::vector<FP> ptcl;
  ...
  for (PS::U32 j = 0; j < ptcl.size(); ++j) {
    ptcl[j].mass =
        ptcl[j].mass * box->x * box->y * box->z / (PS::F64)(ptcl.size());
  }
...
void InitialKick(PS::ParticleSystem<FP>& sph_system, const PS::F64 dt) {
  for (PS::S32 i = 0; i < sph_system.getNumberOfParticleLocal(); ++i) {
    ...
    sph_system[i].eng_half =
        sph_system[i].eng + 0.5 * dt * sph_system[i].eng_dot;
  }
}
...
void FinalKick(PS::ParticleSystem<FP>& sph_system, const PS::F64 dt) {
  for (PS::S32 i = 0; i < sph_system.getNumberOfParticleLocal(); ++i) {
    ...
    sph_system[i].eng =
        sph_system[i].eng_half + 0.5 * dt * sph_system[i].eng_dot;
  }
}
